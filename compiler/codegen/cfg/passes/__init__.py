...
三段共享**同一个** `CfgCtx`（在 `lower/cfg_ctx.py`，形态对齐中端 `analysis/lowering/sem_ctx.py`）：
session 级资源构造即定，函数表（`ctx.functions`）与每函数事实（`ctx.facts(type_id)`）由下降段写入，
之后三段读的是同一份——所以任一段都能看到本模块的全部函数。
