...
class ClosureLowering:
    ...
    def __lower_one(self, dp: DefPoint, closure_ty: Type.ClosureType) -> None:
        ...
        self.__setup_rewrite(capture_sids, closure_ty.captured_vars, struct_type_id, self_ref_type_id, self_sid)
    ...
    def __rewrite_type_ids(self, expr: HIR.Expr) -> HIR.Expr:
        ...
        match expr:
            ...
            case HIR.ArrayAccess():
                expr.array = self.__rewrite_type_ids(expr.array)
    ...
    def __setup_rewrite(self, capture_sids: dict[str, int], captured_vars: list[Type.CapturedVar], struct_type_id: int, self_ref_type_id: int, self_sid: int) -> None:
        ...
        self.__self_ref_type_id = self_ref_type_id
    ...
    def __rewrite_one_capture(self, expr: HIR.Expr) -> HIR.Expr:
        if isinstance(expr, HIR.Var) and expr.symbol_id in self.__sid_to_field:
            ...
            # self is RefType(struct); deref to get the struct value
            self_val = HIR.Var(
                span=expr.span,
                symbol_id=self.__self_sid,
                type_id=self.__self_ref_type_id,
                is_place=True
            )
        ...
        match expr:
            ...
            case HIR.FieldAccess():
                expr.receiver = self.__rewrite_one_capture(expr.receiver)
            case HIR.ArrayAccess():
                expr.array = self.__rewrite_one_capture(expr.array)
                expr.index = self.__rewrite_one_capture(expr.index)
            case HIR.SliceAccess():
                expr.slice = self.__rewrite_one_capture(expr.slice)
                expr.index = self.__rewrite_one_capture(expr.index)
            case HIR.Tuple():
