...
def __export_expr(expr: HIR.Expr, guides: list[bool], is_last: bool, type_ctx: TypeCtx | None) -> str:
    match expr:
        ...
        case HIR.SliceAccess():
            return __export_slice_access(expr, guides, is_last, type_ctx)
        case HIR.DynValue():
        ...
        case HIR.YianArgc():
            return __line(guides, is_last, f"YianArgc: type={__format_type(type_ctx, expr.type_id)} span={__format_span(expr.span)}")
        case HIR.YianArgvPtr():
            res = __line(guides, is_last, f"YianArgvPtr: type={__format_type(type_ctx, expr.type_id)} span={__format_span(expr.span)}")
            res += __export_expr_child("Index", expr.index, guides, is_last, True, type_ctx)
            return res
        case HIR.YianCstrlen():
            res = __line(guides, is_last, f"YianCstrlen: type={__format_type(type_ctx, expr.type_id)} span={__format_span(expr.span)}")
            res += __export_expr_child("Ptr", expr.ptr, guides, is_last, True, type_ctx)
            return res
        case HIR.YianExit():
...
def __export_slice_access(expr: HIR.SliceAccess, guides: list[bool], is_last: bool, type_ctx: TypeCtx | None) -> str:
    res = __line(guides, is_last, f"SliceAccess: element_type={__format_type(type_ctx, expr.element_type)} place={expr.is_place} span={__format_span(expr.span)}")
    res += __export_expr_child("Slice", expr.slice, guides, is_last, False, type_ctx)
    res += __export_expr_child("Index", expr.index, guides, is_last, True, type_ctx)
    return res
