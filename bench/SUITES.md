...
本文件登记仓库内评测集的**来源与许可**，并登记已调研的候选套件（含许可核实结论、内容、
与 YIAN 的相关性、引入工作量）作为引入判据。评测集的组织方式与运行方式见 `bench/README.md`。
...
| 本仓库自写（方向 B 机制微基准：call_abi、copy_struct、chase） | `bench/fatptr/` | 3 | 本仓库许可；只用于表示尺寸/拷贝/追逐的量级读数，不参与对外结论 |
| 本仓库自写（C 参考） | `bench/AWFY/c/`、`bench/BG/c/`、`bench/fatptr/c/` | 22 | 本仓库许可；与对应 `.an` 同算法同规模 |

## 二、候选套件（许可已核实）

| 候选 | 内容 | 许可（已核实） | 相关性 | 结论 |
| --- | --- | --- | --- | --- |
| **plb2**（`attractivechaos/plb2`） | 每个语言 4 个程序：`matmul`、`nqueen`、`sudoku`、`bedcov`；C 参考齐全 | **CC0-1.0**（公共领域奉献） | 补"数值 + 回溯搜索 + 哈希/区间覆盖"侧重；体量小、单线程 | matmul/nqueen/sudoku 满足引入硬条件；bedcov 需要文件 I/O，暂不纳入 |
| **Benchmarks Game 其余 5 项** | k-nucleotide、mandelbrot、n-body、pidigits、regex-redux | Revised BSD（与已引入部分同源） | k-nucleotide 补"哈希 + 字符串"；mandelbrot/n-body 与 AWFY 的 mand/nbody 重复；pidigits 需大数、regex-redux 需正则引擎 | k-nucleotide 满足引入硬条件；其余在标准库提供大数与正则之前不纳入 |
| **mimalloc-bench**（`daanx/mimalloc-bench`） | 分配器压力测试若干；单线程可用者如 `alloc-test`、`rptest`、`sh6bench`/`sh8bench`、`xmalloc-test`；`larson`/`mstress` 是多线程 | **MIT** | 补"随机尺寸分布"的分配压力（现有 4 件是固定/两类尺寸） | 单线程项（如 1–2 个）可作为 `bench/ALLOC` 的补充 |
| **PolyBench/C**（经 LLVM test-suite 分发） | ~30 个数值内核（线性代数 / 数据挖掘 / 模板计算 / medley） | LLVM test-suite = Apache-2.0 with LLVM exception；Polybench 目录自带 LICENSE | 与 `numeric` 侧重部分重叠，但补"长数组 + 边界检查密集" | 可作为 `numeric` 补充（如 gemm、2mm、atax、jacobi-2d、fdtd-2d） |
| **Olden**（`compor/olden`；LLVM test-suite 亦有一份） | 10 个指针/递归密集 C 程序：`bh`、`bisort`、`em3d`、`health`、`mst`、`perimeter`、`power`、`treeadd`、`tsp`、`voronoi` | **不可直接 vendor**：LICENSE 为 LLVM + 原始非商业限制（"不得与商业产品或服务一起分发"） | 与方向 A/B 最相关的侧重（对象图、递归、分配） | 不 vendor：只能按同样算法**自写** YIAN + C 参考，或放在 `build/` 下作为外部可选套件（不进仓库） |
| **MiBench** | 35 个嵌入式 C 程序（汽车/消费/办公/安全/网络/通信） | 站点声明"free for academic use"，具体条款需人工确认 | 与指针/内存机制相关性低 | 许可与相关性都不占优，不纳入 |
| **Embench-IoT** | 19 个嵌入式基准 | **GPL-3.0** | 相关度低 | 与本仓库 Apache-2.0/MIT 双许可不兼容，不引入 |
