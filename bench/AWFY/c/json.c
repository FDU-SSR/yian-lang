// bench/AWFY/c/json.c — JSON 解析 (Json, AWFY 宏基准) 的 C 参考实现
//
...
static int verify_document(const Jv *root) {
    if (root == NULL || root->kind != JV_OBJECT) {
        return 0;
    }
    int32_t head_index = index_of(&root->names, &root->table, str_make("head", 4));
    if (head_index < 0 || root->values.items[head_index]->kind != JV_OBJECT) {
        return 0;
    }
    return ops_size(root) == 156u;
}
...
int main(int argc, char **argv) {
    int iterations = argc > 1 ? atoi(argv[1]) : 14286;
    for (int i = 0; i < iterations; i++) {
        ...
        assert(verify_document(root) && "json verification failed");
        jv_free(root);
        free(p.cbuf);
        for (size_t j = 0; j < p.arena_len; j++) {
            free(p.arena[j]);
        }
        free(p.arena);
    }
