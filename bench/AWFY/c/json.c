// bench/AWFY/c/json.c — JSON 解析 + 序列化 (Json, AWFY 宏基准) 的 C 参考实现
//
// 语义与规模与 bench/AWFY/an/json.an 对齐; 只用于跨语言正确性对照, 不参与 fat/raw 计时。
...
//       HashIndexTable。校验值: operations 数组长度 156 (上游官方期望值), 序列化
//       (minified) 长度 25820, 序列化字节的 FNV-1a 32 = 2934551514; 输出为三者的
//       ITER(=2000) 倍累加值, 与 .an 完全相同:
//       stdout == "json 312000 51640000 5869103028000"
...
static Str str_make(const char *ptr, size_t len) {
    Str s;
    s.ptr = ptr;
    s.len = len;
    return s;
...
typedef enum { JV_OBJECT, JV_ARRAY, JV_STRING, JV_NUMBER, JV_LITERAL } JvKind;
...
static void str_vec_push(StrVec *v, Str value) {
    if (v->len == v->cap) {
        v->cap = v->cap == 0 ? 4 : v->cap * 2;
        v->items = (Str *)xrealloc(v->items, v->cap * sizeof(Str));
    }
    v->items[v->len] = value;
    v->len += 1;
}

static void jv_vec_push(JvVec *v, Jv *value) {
    if (v->len == v->cap) {
        v->cap = v->cap == 0 ? 4 : v->cap * 2;
        v->items = (Jv **)xrealloc(v->items, v->cap * sizeof(Jv *));
    }
    v->items[v->len] = value;
    v->len += 1;
}

static void i32_vec_push(I32Vec *v, int32_t value) {
    if (v->len == v->cap) {
        v->cap = v->cap == 0 ? 32 : v->cap * 2;
        v->items = (int32_t *)xrealloc(v->items, v->cap * sizeof(int32_t));
    }
    v->items[v->len] = value;
    v->len += 1;
}

static Jv *jv_new(JvKind kind) {
...
// 上游 JsonLiteral / JsonString / JsonNumber 的工厂
static Jv *jv_literal(const char *text) {
    Jv *v = jv_new(JV_LITERAL);
    v->text = str_make(text, strlen(text));
    return v;
}

// ── HashIndexTable (上游 JsonObject.HashIndexTable) ──

static uint64_t hash_slot(Str name) {
    return ((uint64_t)name.len * 1402589u) & 31u;
}

static int32_t index_of(const StrVec *names, const I32Vec *table, Str name) {
    uint64_t slot = hash_slot(name);
    int32_t idx = (table->items[slot] & 0xff) - 1;
    if (idx != -1 && str_eq(names->items[idx], name)) {
        return idx;
    }
    return -1;
}

// ── 解析器 (上游 JsonPureStringParser) ──
...
static void cb_append(Parser *p, const char *data, size_t len) {
    if (p->cbuf_len + len > p->cbuf_cap) {
        size_t cap = p->cbuf_cap == 0 ? 32 : p->cbuf_cap;
        while (cap < p->cbuf_len + len) {
            cap *= 2;
        }
        p->cbuf = (char *)xrealloc(p->cbuf, cap);
        p->cbuf_cap = cap;
    }
    memcpy(p->cbuf + p->cbuf_len, data, len);
    p->cbuf_len += len;
}

static void arena_push(Parser *p, char *buf) {
    if (p->arena_len == p->arena_cap) {
        p->arena_cap = p->arena_cap == 0 ? 4 : p->arena_cap * 2;
        p->arena = (char **)xrealloc(p->arena, p->arena_cap * sizeof(char *));
    }
    p->arena[p->arena_len] = buf;
    p->arena_len += 1;
}
...
static void p_read(Parser *p) {
    ...
    p->index += 1;
    if (p->index < (int64_t)p->input_len) {
        p->current = (unsigned char)p->input[p->index];
    } else {
        p->current = -1;
    }
}

static int p_read_char(Parser *p, int ch) {
    if (p->current != ch) {
        return 0;
    }
    p_read(p);
...
static int p_is_white_space(const Parser *p) {
    return p->current == 32 || p->current == 9 || p->current == 10 || p->current == 13;
}

static void p_skip_white_space(Parser *p) {
    while (p_is_white_space(p)) {
        p_read(p);
    }
}

static void p_start_capture(Parser *p) {
    p->capture_start = p->index;
}

static void p_pause_capture(Parser *p) {
    int64_t end = p->current < 0 ? p->index : p->index - 1;
    cb_append(p, p->input + p->capture_start, (size_t)(end + 1 - p->capture_start));
    p->capture_start = -1;
}

static Str p_end_capture(Parser *p) {
    int64_t end = p->current < 0 ? p->index : p->index - 1;
    int64_t start = p->capture_start;
    p->capture_start = -1;
    if (p->cbuf_len == 0) {
        return str_make(p->input + start, (size_t)(end + 1 - start));
    }
    cb_append(p, p->input + start, (size_t)(end + 1 - start));
    char *buf = (char *)xrealloc(NULL, p->cbuf_len);
    memcpy(buf, p->cbuf, p->cbuf_len);
    size_t len = p->cbuf_len;
    p->cbuf_len = 0;
    arena_push(p, buf);
...
static void p_read_escape(Parser *p) {
    p_read(p);
    switch (p->current) {
        case 34: cb_append(p, "\"", 1); break;
        case 47: cb_append(p, "/", 1); break;
        case 92: cb_append(p, "\\", 1); break;
        case 98: cb_append(p, "\b", 1); break;
        case 102: cb_append(p, "\f", 1); break;
        case 110: cb_append(p, "\n", 1); break;
        case 114: cb_append(p, "\r", 1); break;
        case 116: cb_append(p, "\t", 1); break;
        default: parse_fail(); break;
    }
    p_read(p);
}

static Str p_read_string_internal(Parser *p) {
    p_read(p);
    p_start_capture(p);
    while (p->current != 34) {
        if (p->current < 0) {
    ...
    Str captured = p_end_capture(p);
    p_read(p);
    return captured;
}

static Jv *p_read_string(Parser *p) {
    Jv *v = jv_new(JV_STRING);
    v->text = p_read_string_internal(p);
    return v;
}

static Str p_read_name(Parser *p) {
    if (p->current != 34) {
        parse_fail();
    }
    return p_read_string_internal(p);
}

static void p_require(Parser *p, int ch) {
    if (!p_read_char(p, ch)) {
        parse_fail();
    }
}

static Jv *p_read_null(Parser *p) {
    p_read(p);
    p_require(p, 117);
    p_require(p, 108);
    p_require(p, 108);
    return jv_literal("null");
}

static Jv *p_read_true(Parser *p) {
    p_read(p);
    p_require(p, 114);
    p_require(p, 117);
    p_require(p, 101);
    return jv_literal("true");
}

static Jv *p_read_false(Parser *p) {
    p_read(p);
    p_require(p, 97);
    p_require(p, 108);
    p_require(p, 115);
    p_require(p, 101);
    return jv_literal("false");
}

static Jv *p_read_value(Parser *p);

static void p_read_fraction(Parser *p) {
    if (!p_read_char(p, 46)) {
        return;
    }
    if (!p_read_digit(p)) {
    ...
    while (p_read_digit(p)) {
    }
}

static void p_read_exponent(Parser *p) {
    if (!p_read_char(p, 101) && !p_read_char(p, 69)) {
        return;
    }
    if (!p_read_char(p, 43)) {
        p_read_char(p, 45);
...
static Jv *p_read_number(Parser *p) {
    p_start_capture(p);
    p_read_char(p, 45);
    int first_digit = p->current;
    if (!p_read_digit(p)) {
        parse_fail();
    }
    if (first_digit != 48) {
    ...
    p_read_fraction(p);
    p_read_exponent(p);
    Jv *v = jv_new(JV_NUMBER);
    v->text = p_end_capture(p);
    return v;
}

static Jv *p_read_array(Parser *p) {
    p_read(p);
    JvVec values = {NULL, 0, 0};
    p_skip_white_space(p);
    if (p_read_char(p, 93)) {
        Jv *v = jv_new(JV_ARRAY);
        v->values = values;
        return v;
    }
    for (;;) {
        p_skip_white_space(p);
        jv_vec_push(&values, p_read_value(p));
        p_skip_white_space(p);
        if (!p_read_char(p, 44)) {
            break;
        }
...
static Jv *p_read_object(Parser *p) {
    p_read(p);
    StrVec names = {NULL, 0, 0};
    JvVec values = {NULL, 0, 0};
    I32Vec table = {NULL, 0, 0};
    for (int k = 0; k < 32; k++) {
        i32_vec_push(&table, 0);
    }
    p_skip_white_space(p);
    if (p_read_char(p, 125)) {
        Jv *v = jv_new(JV_OBJECT);
        v->names = names;
        v->values = values;
        v->table = table;
        return v;
    ...
    for (;;) {
        p_skip_white_space(p);
        Str name = p_read_name(p);
        p_skip_white_space(p);
        if (!p_read_char(p, 58)) {
            parse_fail();
        }
        p_skip_white_space(p);
        // 上游 HashIndexTable.add(name, names.size())
        if (names.len < 255u) {
            table.items[hash_slot(name)] = (int32_t)((int32_t)names.len + 1) & 0xff;
        }
        str_vec_push(&names, name);
        jv_vec_push(&values, p_read_value(p));
        p_skip_white_space(p);
        if (!p_read_char(p, 44)) {
            break;
...
// 上游 JsonPureStringParser.parse (arena 由调用方持有, 保证 Str 生命周期;
// 本文档无转义, arena 始终为空)
static Jv *parse_document(Parser *p) {
    p->index = -1;
    p->line = 1;
    p->column = 0;
    p->current = -1;
...
// ── 序列化 (minified; 与 json.an 的 write_value 逐分支对应) ──
...
static void buf_append(Buf *b, const char *data, size_t len) {
    buf_reserve(b, len);
    memcpy(b->data + b->len, data, len);
    b->len += len;
}

static char hex_digit(unsigned b) {
    return (char)(b < 10u ? ('0' + b) : ('a' + b - 10u));
}

static void write_escaped(Buf *out, Str s) {
    buf_push(out, '"');
    for (size_t i = 0; i < s.len; i++) {
        ...
        if (b == 34u) {
            ...
            buf_append(out, "\\u00", 4);
            buf_push(out, hex_digit(b / 16u));
            buf_push(out, hex_digit(b % 16u));
        } else {
            buf_push(out, (char)b);
        }
...
static void write_value(Buf *out, const Jv *v) {
    switch (v->kind) {
        case JV_OBJECT:
            buf_push(out, '{');
            for (size_t i = 0; i < v->names.len; i++) {
                write_escaped(out, v->names.items[i]);
                buf_push(out, ':');
                write_value(out, v->values.items[i]);
                if (i + 1u < v->names.len) {
                    buf_push(out, ',');
                }
            }
        ...
        case JV_ARRAY:
            ...
            break;
        case JV_STRING:
            write_escaped(out, v->text);
            break;
        case JV_NUMBER:
        case JV_LITERAL:
            buf_append(out, v->text.ptr, v->text.len);
            break;
    }
}

// 序列化字节的 FNV-1a 32 (与 json.an 的 fnv1a32 相同)
...
// ── 上游 Json.verifyResult: operations 必须是 array 且 size == 156 ──

static size_t ops_size(const Jv *root) {
    if (root->kind != JV_OBJECT) {
        parse_fail();
    }
    int32_t idx = index_of(&root->names, &root->table, str_make("operations", 10));
    if (idx < 0) {
        parse_fail();
    }
    const Jv *ops = root->values.items[idx];
    if (ops->kind != JV_ARRAY) {
        parse_fail();
    }
    return ops->values.len;
}

// 递归释放树 (节点内联在父 Vec 中的 C 版本为指针节点, 逐层释放)
static void jv_free(Jv *v) {
    ...
    if (v->kind == JV_OBJECT || v->kind == JV_ARRAY) {
        for (size_t i = 0; i < v->values.len; i++) {
            jv_free(v->values.items[i]);
        }
        free(v->values.items);
    }
    free(v);
...
int main(void) {
    ...
    memset(&p, 0, sizeof(p));
    p.input = DOC;
    p.input_len = strlen(DOC);
    p.index = -1;
    p.line = 1;
    p.current = -1;
    p.capture_start = -1;

    assert(p.input_len == DOC_LEN);
    ...
    for (uint64_t i = 0; i < ITER; i++) {
        Jv *root = parse_document(&p);
        total_ops += (uint64_t)ops_size(root);
        out.len = 0;
        write_value(&out, root);
        total_len += (uint64_t)out.len;
        total_hash += (uint64_t)fnv1a32(out.data, out.len);
        jv_free(root);
    }

    assert(total_ops == (uint64_t)EXPECTED_OPS * ITER);
    assert(total_len == (uint64_t)EXPECTED_LEN * ITER);
    assert(total_hash == (uint64_t)EXPECTED_HASH * ITER);

    free(out.data);
    free(p.cbuf);
    for (size_t i = 0; i < p.arena_len; i++) {
        free(p.arena[i]);
    }
    free(p.arena);

    printf("json %llu %llu %llu\n",
