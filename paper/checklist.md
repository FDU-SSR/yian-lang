...
- [ ] 在指定实验机器重测 14×3：CPU 4，warmup 1，正式样本 5，三态紧邻。
- [ ] 在同一机器重测 C plain/C ASan/SecL check：43 条腿，每腿 5 个样本。
- [ ] 同步 paper/data 冻结快照；当前数据是帧锁改动前的历史快照。

## 图表

- [ ] 用重测数据生成三态成本分解 PDF，并核对 14 行断言。
- [x] 生成 38-CVE 机制汇总图；完整矩阵只作为候选附录图。
- [ ] 生成 ASan 候选图并核对重测报告中的三口径几何平均。
