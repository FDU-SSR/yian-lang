...
2026-08-25 在提交 `e8a8c115` 后的冻结结果：
...
| 口径 | 14 项几何平均 | 范围 | ≤1.2× |
| --- | ---: | ---: | ---: |
| 表示/池/锁成本 `nocheck/raw` | 1.16× | 0.77--3.12× | 9/14 |
| 检查发射成本 `check/nocheck` | 1.10× | 0.96--1.58× | 11/14 |
| 完整机制 `check/raw` | 1.27× | 0.97--3.44× | 7/14 |

最差总时间倍率为 `list` 3.44×，其中表示/池成本 3.12×、检查成本 1.10×；`queen`、
`revcomp` 和 `towers` 分别为 1.57×、1.57× 和 1.59×。小于 1 的倍率只应解释为代码生成、
源码适配和测量噪声的组合，不能声称安全机制带来加速。

RSS 的主要异常是 `binarytree` +255.94MB 和 `storage` +2189.47MB。前者受树节点表示和不同
...
- C ASan / C plain 几何平均 1.56×；
- C ASan / SecL check 几何平均 0.70×；
- SecL check / C plain 几何平均 2.23×。

这些是端到端跨编译器结果：C plain/ASan 用 clang `-O2`，SecL 用自身前端和 LLVM/clang
`-O3`，源代码表示也不同。因此只有第一项可近似解释为 ASan 在同一 C 程序上的插桩成本；
后两项不得全部归因于检查机制。

`binarytree` 在默认 quarantine 下的 ASan RSS 为 647.9MB；关闭 quarantine 后为 166.7MB，
下降 74.3%，时间下降 9.5%。SecL 稳定池和 ASan quarantine 解决的问题与释放策略不同，不应
用单一 RSS 数字推导覆盖能力。
