...
**规则 3.5.1（局部变量取址 `&x`）**（→ CFG `VarPtr` / `Alloca`）。前提：`x` 为当前帧 `f` 的局部变量（帧锁 ⟨e_f, k_f⟩，定义 15）。动作：取 `x` 的帧栈地址 `a_x`（CFG 层由 `Alloca` 分配栈槽、`VarPtr` 取槽地址）。结果：`x` 得 `⟨a_x, e_f, k_f, 0, 1⟩`——取址总是指向单个元素（标量元素类型 $T$、数组元素类型 $T[m]$），数组取址可 coerce 到首元素 $T^{*}$（§2.7）。取址不访问内存，无 trap 前提。

**规则 3.5.2（字段取址 `&s.field`）**（→ CFG `FieldPtr`）。前提：`in_bounds(p_s, 1)`（`s` 整体在界内，非 one-past-end）；设字段 `field : T'` 在 `s` 内的字节偏移为 `δ`（由类型布局确定，`0 ≤ δ` 且 `δ + |T'| ≤ |S|`）。动作（子对象锚定，§2.4）：`data' := addr_T(p_s, 0) + δ`（字段字节基址），`index' := 0`，`size' := 1`（字段取址指向单个字段元素，元素类型 $T'$）；`lock_ptr`/`key` 继承自 `s`（同对象共享锁槽）。结果：`⟨data', p_s.lock_ptr, p_s.key, 0, 1⟩`。若前提不满足则 trap（对 one-past-end 的 `s` 取字段）。
