...
失败轨迹叙述：类 1 的 `p[3]` 在无检查的裸指针语义下读得 `arr` 之后相邻内存的内容，结果取决于布局巧合；在修订后设计中 `p[3]` 的 `in_bounds(p, 4)` 检查（定义 12）先失败，程序于访问前 trap。类 2 的 `*q` 在无检查语义下可能读到被重新分配块的脏数据或恰好复活的旧值；修订后 `live(q)`（定义 8）经块内锁槽寻址，`q` 释放时锁槽已写 `SENTINEL`，检查失败即 trap。类 3 的 `bitcast` 在现状原型中合法（`expr_checker.py:132-157`），第 8 章收紧后按布局兼容前提（定义 27、规则 8.3.1）在编译期拒绝。类 4 的 `s + 0xFFFFFFFFFFFFFFFF` 在 64 位回绕下得回块内地址、躲过界内检查；修订后 `index + n` 按数学整数求值（§2.4 约定），结果 $0 + 2^{64}-1$ 远超 `size = 1`，在算术规则处 trap。
...
- **不可绕过检查**：攻击者不能直接改写锁槽（锁槽内置于块头/帧、由运行时独占维护并释放写哨兵，见 §2.3），也不能凭空构造一个键与活动锁槽匹配的胖指针——胖指针只能由分配、取址、数组退化等良构操作产生（第 8 章类型约束）。
...
**隔离假设（时序行论证前提）**：时序检查可能触碰已释放块的锁槽，其值确定性为 `SENTINEL`（隔离假设：释放后内存保持映射可读，§2.2/§5.5/§11.1）——该读不产生不确定结果，故不削弱时序防护的确定性。
...
**存储域与锁槽域的关系（隔离假设）**：$\mathrm{dom}(\mu)$（已分配位置）包含锁槽区：锁槽物理存储于块头/帧内，属 $\mathrm{dom}(\mu)$ 的保留子域，由运行时独占维护（定义 1 注）。$\mu$ 的域按分配/释放增减（规则 3.6.1-3.6.2、3.8.1-3.8.2），锁槽区在隔离期内仍留在 $\mathrm{dom}(\mu)$ 的保留子域中（与定义 4 兼容——未定义位置读为 trap 的前提不适用于锁槽保留子域）。因此 `live(p)` 可能触碰已释放块的锁槽 $\Lambda(p.\text{lock\_ptr})$，其值确定性地为哨兵（`SENTINEL`，定义 6），检查自身不产生不确定读（缺陷 1 的修订落点）。
...
锁槽物理存储于被守护块的块头或栈帧内（逻辑上仍以 $\Lambda$ 表示，谓词/引理结构不变）。锁槽随块存在：堆块释放与栈帧退出把对应锁槽写成 `SENTINEL`（定义 6），块入隔离期（§2.5）锁槽区保持可读——时序检查 $\Lambda(p.\text{lock\_ptr})$ 的查找读得确定性哨兵（隔离假设，§2.2），永不返回不确定脏值。

**锁槽工作示例**：设当前 $\Lambda = \{ e_0 \mapsto \langle \text{heap}, k_0 \rangle,\ e_1 \mapsto \text{SENTINEL} \}$（$e_0$ 位于某活动堆块块头、$e_1$ 位于某已释放块的块头）。指针 `p` 携带 `lock_ptr = e_0`、`key = k_0`：`live(p)` 为真；`lock_ptr = e_1` 的任何指针：锁槽值为 `SENTINEL`，`live` 为假。`e_1` 所在块被新分配复用后锁槽重写为 `⟨heap, k_1⟩`（$k_1 \ne k_0$，定义 10），携带旧键的指针仍不匹配。`is_heap(p)`（定义 9）与 `live(p)` 都只读 `Λ(p.lock_ptr)`，一次读取同时完成 kind 判定与键比较，`Delete` 节点的三前提检查（§7.1）可合并为对同一锁槽的一次读取。
...
协议性质：由于键唯一性（定义 10 推论），锁槽即使被新分配复用也只写新键，旧指针的键永不重新匹配（锁不复用，第 4 章引理）；双释放与 UAF 均由 `live` 前提捕获。对重锚定子对象指针（如 `&s.field`）执行 `delete` 时，语义为释放其所属的整个分配，随后该分配的一切派生指针（无论锚定方式）均时序失效。协议性质的成因：`delete` 动作是「锁槽写 `SENTINEL` + 块入隔离」两步的原子组合，写哨兵发生在入隔离之前（规则 3.6.2 动作①②），故即使释放后分配器立即复用该地址，锁槽已写哨兵或新键、不匹配旧键，派生指针的时序失效与地址是否复用无关。
...
2. 将帧栈块的区间自 $\mathrm{dom}(\mu)$ 撤销（栈内存恒映射，读死帧锁槽确定性得哨兵）；
...
表示层共同约定：胖指针在存储中以 `Val` 的 `PtrVal` 分量存放；其 `lock_ptr` 指向块头/帧内锁槽，锁槽元数据与被保护数据相邻存储。该布局下「内联元数据可伪造」弱点的缓解不依赖存储位置隔离，而依靠哨兵机制与 §1.2 无任意写前置：攻击者无任意写即无法在释放前改写锁槽，释放后锁槽读得确定性哨兵（与 §6.5/§11.1 一致）。
...
**规则 3.8.2（帧退出 `exit f`）**（帧级，CFG 无对应节点）。前提：无。动作（§2.6）：帧锁槽写哨兵 `Λ(e_f) := SENTINEL`；帧栈块区间自 `dom(μ)` 撤销（栈内存恒映射，读死帧锁槽确定性得哨兵，§2.6）。结果：帧内产生的全部指针此后 `live` 恒为假；对已退出帧地址的任何后续访问在访问规则处 trap（栈悬垂防护）。
...
本章把第 2-3 章表示与规则中的安全性要求升格为可陈述、可引用的不变量与引理。第 5 章主可靠性定理以 S1、T1、T-TYPE 三条不变量为直接前提，以 L-KEY、L-SENTINEL、L-NOREUSE、L-REKEY 为中间结论支撑 T1 的作废永久性一侧（隔离期由哨兵不匹配闭合，堆/栈复用期由键唯一性闭合）。每条 = 陈述（闭公式）+ 论证梗概 + 证明义务，不给出 PLDI 级完整证明（明确排除）；论证范围限于第 1 章 §1.3 的 in 类程序：单线程、不跨 FFI、攻击者无任意写前置，且 `bitcast` 受第 8 章约束。符号全部沿用第 2-3 章（定义 1-19），不重复定义；新定义自编号 20 起。
...
关键步骤展开：正向侧的子义务 O-2a 即逐条枚举 3.2.1-3.2.4、3.10.1、3.6.2 的正常前提并指出其中含 `live` 的显式出现，这是规则文本的直接核对，无推理跳跃。负向侧的 O-2b 需三种「复活」窗口都被切断：隔离期窗口——释放/帧退出后锁槽值为 `SENTINEL`，任何指针键与之失配（L-SENTINEL，§4.4）；堆复用窗口——堆锁槽被新分配复用（L-NOREUSE 断言新键 ≠ 旧键）；栈复用窗口——栈锁槽被递归/循环帧复用（L-REKEY 断言两轮进入的键不同）。三条路径的共同结构是「锁槽可复用，但键不可复用」（`SENTINEL` 不是键、更不可复用），作废永久性的来源因此是键唯一性与哨兵值域而非锁槽唯一性。派生指针的枚举沿 §2.4 锚定规则与 §2.7 派生关系闭合：算术只改 `index`、取址继承锁字段、`bitcast` 不改锁字段，故同对象全部指针共享 `lock_ptr`/`key`，一并失效。
...
对照说明（非规则）：旧实现 `__key_lock_match`（`ptr.an:46-49`）读取块内联的 `MemoryBlock.lock` 比较，属内联元数据方案；修订后 `live(p)` 经块内/帧内锁槽寻址（定义 7-8），差异见 §3.11，本章不采用旧方案。对照的边界：T1 的陈述与证明只引用定义 8 的块内/帧内锁槽寻址，旧实现的「读取块内锁」路径在证明中不存在；其失败机制（释放后锁槽读得未定义脏值）正是缺陷 1 的修订对象（§6.1）——方案 2 下 `live(p)` 可能触碰已释放块的锁槽，但其值确定性地为哨兵（隔离假设，§2.2），检查建立在确定性读上，该路径不进入 T1 的前提或结论。
...
**论证梗概**：$t_2$ 时 $e$ 由分配/重分配规则（3.6.1、3.7.1）重新激活，写入的 $k'$ 是 `Gen` 的新调用输出；由 L-KEY(iii)，$k'$ 与 $e$ 的全部历史键不同，特别 $k' \ne k$。此后 $\Lambda_s(e)$ 只可能为 $\langle \text{heap}, k' \rangle$、$\langle \text{stack}, \cdot \rangle$ 或 `SENTINEL`，永不等于 $\langle \text{kind}, k \rangle$，故 $\mathrm{live}(p)$（定义 8）恒为假。关键步骤：作废永久性的来源是键唯一性（作废的键永不重新写入）与哨兵值域（隔离期读得哨兵），而非锁槽不复用。

关键步骤展开：推论「对任意 $s \ge t_2$，$\neg \mathrm{live}_s(p)$」的证明按 $\Lambda_s(e)$ 的三种可能逐情形穷举：若 $e$ 处于活动态，其存储键为 $k'$ 或更晚的重激活键，均不等于 $k$；若 $e$ 为 `SENTINEL`，定义 8 右端不成立（L-SENTINEL）；无第四种可能（定义 6 的值域闭合）。故旧指针的失效与锁槽当前状态无关，是永久性的。注意本引理允许 $e$ 在 $t_1'$ 与 $t_2$ 之间被反复作废/重激活，只要每次重激活都写新键，推论对任意后续时刻成立；$t_1'$ 至 $t_2$ 之间的隔离期窗口由 L-SENTINEL 闭合（锁槽读得 `SENTINEL`，任何指针键失配），作废自 $t_1'$ 即开始。这正是锁槽随块驻留（定义 7）与隔离假设（§2.2）下「释放写哨兵、复用写新键」设计的落点。

**义务 O-4**（T1 负向侧的堆部分）：以 L-KEY 证明作废锁槽复用只获新键、旧指针键永不重新匹配；证明 $e$ 重新激活后至多存贮新键或 `SENTINEL`（写入规则枚举）。

**义务 O-4 的验证次序**：先枚举 $\Lambda$ 的全部写入点（分配 3.6.1、释放 3.6.2、重分配 3.7.1、帧进出 3.8.1-3.8.2），确认其中写活动键者必先 `Gen()`；再对每条重激活路径（3.6.1 的新鲜锁槽与复用 `SENTINEL` 锁槽、3.7.1 的新锁槽）应用 L-KEY(iii) 得新键 ≠ 全部历史键；最后穷举 $\Lambda_s(e)$ 的三值域（定义 6）证明推论。次序固定后，O-4 可机械执行，无开放性反例分支。
...
| O-2b | T1 负向侧；L-SENTINEL（§4.4）+ L-NOREUSE（§4.5）+ L-REKEY（§4.6） | G2（时序） | 作废永久性：隔离期哨兵失配（L-SENTINEL）+ 堆/栈复用期新键失配（L-NOREUSE/L-REKEY） |
...
**O-2b（T1 负向侧 → G2）**：作废永久性由双窗口闭合。隔离期窗口——释放/帧退出后、锁槽复用前，锁槽被写为 `SENTINEL`（规则 3.6.2 动作①、3.8.2），L-SENTINEL（O-3d，§4.4）保证任何指针键与哨兵失配，故该窗口内全部派生指针 `live` 恒假；该读是确定性的（隔离假设，§2.2），不依赖分配器行为。复用期窗口——堆锁槽重激活由 L-NOREUSE（O-4）、栈锁槽复用由 L-REKEY（O-5）保证新键 ≠ 旧键，遗留指针不因锁槽重激活而重新获得时序有效性。两窗口拼接覆盖作废之后的全部时间，使 G2 的排除在时间上封闭：一次作废即永久作废，任何后续访问与重复释放均在该事件处 trap，栈悬垂同理（帧退出后 `live` 恒假）。该义务是 T1 的实质部分，其与正向侧的分工是：正向侧管「检查当时通过」，负向侧管「失效不可逆」，二者合起来才构成 G2 的完整排除。
...
**O-4（L-NOREUSE → T1 堆侧）**：设堆锁槽 $e$ 在 $t_1$ 活动、$t_1'$ 写哨兵（规则 3.6.2 动作①）、$t_2$ 重激活，由 O-3c 得新键 $k' \ne k$（旧键）；此后 $e$ 至多存贮 $\langle \text{heap}, k' \rangle$、$\langle \text{stack}, \cdot \rangle$ 或 `SENTINEL`（写入规则枚举），永不等于 $\langle \text{kind}, k \rangle$。故携带 $\langle e, k \rangle$ 的旧指针此后 `live` 恒假，释放后的堆 UAF 与双释放被排除——即 §2.5「锁不复用」协议的形式化。
...
2. **T1（O-2a ∧ O-2b）排除时序错误 → G2**：正向侧（O-2a）保证成功事件在访问时刻 `live`；负向侧（O-2b）经 L-SENTINEL、L-NOREUSE/L-REKEY 保证作废永久性（隔离期哨兵失配 + 复用期新键失配），给出「无 UAF、双释放、栈悬垂访问正常完成」。
...
**修订后机制**：锁槽随块 + 释放写哨兵 + 隔离假设。定义 6/7 声明锁槽物理存储于块头/帧内，释放/帧退出把锁槽写成 `SENTINEL`（§2.5 步骤 1、规则 3.6.2 动作①、3.8.2）；`live(p)`（定义 8）经块内/帧内锁槽寻址，释放后锁槽读得确定性哨兵而非不确定脏值，检查判定建立在确定性读上（隔离假设，§2.2）。作废永久性由 L-SENTINEL（隔离期，§4.4）→ L-NOREUSE（复用期，§4.5）保证：锁槽重激活只获新键，旧键永不重新匹配。修订的落点：表示层锁槽仍随块（与初版同址），但检查路径从「读块头字段的未定义脏值」变为「读确定性哨兵 + 键唯一性闭合复用窗口」——缺陷 1 的自我引用（检查读已释放内存得到不确定结果）由哨兵确定性读消除。

**理由**：检查读已释放内存不可完全避免（锁与块同址是方案 2 的前提），但该读必须是确定性的：释放写哨兵使锁槽值在隔离期内恒为 `SENTINEL`（隔离假设，§2.2），检查判定建立在确定性读上；复用期由键唯一性闭合。修订是确定性读 + 键唯一性的结构性组合。
...
**处置：收敛论证（不修订）**。成立边界：仅在 §1.2「无任意写前置」假设内成立，即攻击者不能改写锁槽、指针元数据或对象数据，除非通过运行时检查；该假设是威胁模型的 in/out 边界（§1.3 元数据行显式为 out）。在此假设内元数据不可伪造：胖指针只能由分配、取址、数组退化等良构操作产生（第 8 章类型约束），`size` 由 §2.4 锚定规则建立并沿算术/`bitcast` 保持（良构性，定义 13）；锁槽由运行时独占维护、随块头建立、释放写哨兵（§2.7 表示层约定），攻击者无任意写即无法在释放前改写锁槽，释放后锁槽读得确定性哨兵。「改写元数据绕过检查」的路径被假设排除，不构成 in 类论证的反例（§1.3 说明、§5.5 前置条件声明）。
...
7. **隔离假设/隔离池**：释放后内存保持映射可读（隔离假设，§2.2）使已释放块的锁槽读得确定性哨兵，代价是释放后内存不归还 OS 的驻留成本与隔离池管理（已释放块的追踪与复用候选选取）；若实现选择 `munmap` 归还物理页，读锁槽由 trap 退化为段错误，属实现选择边界。对应 §11.1 局限。
...
**元数据防伪依赖 §1.2 假设（out）**：锁槽在块内（块头/帧）、防伪依哨兵 + §1.2 假设：释放后锁槽读得确定性哨兵，攻击者无任意写即无法伪造。但 `size`/`index` 仍作为胖指针字段存放于存储（§6.5）。「元数据不可伪造」仅在 §1.2「无任意写前置」假设内成立：攻击者不能改写锁槽或指针元数据，除非通过了运行时检查。若攻击者获得一次任意写前置，即可改写 `size` 扩大访问或改写 `lock_ptr`/`key`，检查被废除；该失效属 out 行边界（§1.3），不构成 in 类论证的反例（§5.5）。side-table 变体（§11.2）是解除该假设依赖的方向。

**隔离假设（out，实现选择边界）**：本机制要求释放后内存保持映射可读（不 `munmap`，§2.2）：释放协议写 SENTINEL 后块入隔离、内存不归还 OS，使隔离期内锁槽读得确定性哨兵。若实现选择把释放内存归还操作系统（`munmap`），读已释放锁槽由 trap 退化为段错误，属实现选择边界（§5.5 论证范围声明）。
...
| `lock_ptr` | `u64*`，指向锁的指针；锁在块生命周期开始分配、结束时释放 | 指向块内/帧内锁槽（定义 7）；释放/帧退出写 SENTINEL | 缺陷 1（§6.1）：锁随块释放使检查读已释放内存；释放写 SENTINEL 使释放后锁槽读得确定性哨兵，复用窗口由键唯一性闭合 |
