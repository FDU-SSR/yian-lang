...
胖指针按静态类型分三级表示（精确形式化见第 2 章 §2.4，此处分档列名）：

- **`T*`（指针）**：5 字段 40B，`{data, lock_ptr, key, index, size}`
- **`T[]`（切片，`str` 同构）**：4 字段 32B，`{data, lock_ptr, key, size}`——删 `index`，`data` 即有效地址
- **`T&`（引用）**：3 字段 24B，`{data, lock_ptr, key}`——删 `index` 与 `size`，恒指向单个元素

字段共义（各级保留的字段）：

- `data: T*`: 指向实际内存对象负载区的指针
- `lock_ptr: u64*`: 指向锁的指针
- `key: u64`: 锁的密钥
- `index: u64`（仅 `T*`）: 实际内存对象相对原始地址的偏移，单位是元素个数
- `size: u64`（仅 `T*`/`T[]`）: 内存块内存储的元素个数，用于防止越界访问
...
**攻击场景叙述**：在上述假设内，攻击者能控制输入数据、能选择程序路径，但每次内存访问都被检查：越界写要么在 `in_bounds`（定义 12）处 trap、要么落在界内，攻击者无法改写锁槽或任何指针的 `size`（元数据防伪假设）；UAF 访问在 `live`（定义 8）处核对锁槽键，释放即写哨兵、永不复活（L-NOKEY）。攻击者唯一能影响的是「检查是否触发」，而非「触发后是否被放行」；trap 吸收态（§2.1）使检查失败后无任何后续访问可执行。
...
| FFI    | 跨 FFI 边界的裸指针越界 | out  | 检查不跨越 FFI；系统调用类受限操作（`sys_read`/`sys_write`/`open`/`close`）禁于标准库外（《编译器实现》第 8 章）；运行时入口 FFI（`args`/`exit`/`__yian_*`）已移除，仅余系统调用边界；为局限（《编译器实现》第 11 章） |
...
**存储域与锁槽域的关系（释放后立即复用约定）**：锁槽是 $\mathrm{dom}(\mu)$ 中的普通内容（定义 7）。释放/帧退出写 `SENTINEL` 后，锁槽立即由分配器/复用方支配（写入新键或任意值）；锁槽始终被已分配值占据，故 `live(p)` 读锁槽总是读到某个已定义值，「未定义位置读为 trap」条款对锁槽不触发。锁槽位置在复用后的取值由复用方决定，其时序安全性由 §2.3「锁槽复用」集中说明闭合（值失配，L-NOKEY）。
...
**定义 8（时序有效性谓词 `live`）**：指针 $p = \langle \text{data}, \text{lock\_ptr}, \text{key}, \ldots \rangle$（三级表示的公共三元组，定义 11；`T*` 后接 `index`/`size`，`T[]` 后接 `size`，`T&` 仅此三元）在时刻 $t$ 时序有效，当且仅当 $p.\text{lock\_ptr}$ 指向的块头/帧首锁槽（定义 7）中存储的值与指针携带的键一致（全字相等）；锁槽物理值 $v$ 与指针键 $p.\text{key}$ 不等（含哨兵值与任意非键值，值失配）时恒判假：
...
**定义 11（胖指针 `PtrVal`）**：胖指针按静态类型分三级表示（tiered pointers，2026-08）：

- **指针 `T*`**：五元组 $p = \langle \text{data}, \text{lock\_ptr}, \text{key}, \text{index}, \text{size} \rangle$，40B；
- **切片 `T[]`**（`SliceType`；`str` 同构）：四元组 $p = \langle \text{data}, \text{lock\_ptr}, \text{key}, \text{size} \rangle$，32B——删去 `index` 字段（切片恒指向对象首元素，`data` 即有效地址）；
- **引用 `T&`**（`RefType`）：三元组 $p = \langle \text{data}, \text{lock\_ptr}, \text{key} \rangle$，24B——删去 `index` 与 `size`（引用恒指向单个元素，无越界概念）。
...
| `T[]` | `data`     | `Addr`（`T*`）   | 字节         | 负载区锚地址 = 对象首元素地址（`index` 已折叠进 `data`）              | 由 `T*` coerce 降级（§2.7），锁继承源指针                         |
| `T[]` | `lock_ptr` | `Addr`（`u64*`） | 无（地址）   | 同 `T*`：锁槽地址（锁继承源指针）                                     | 由 `T*` coerce 降级（§2.7）                                      |
| `T[]` | `key`      | `Key`（`u64`）   | 无（键值）   | 同 `T*`：时序键（锁继承源指针）                                       | 由 `T*` coerce 降级（§2.7）                                      |
| `T[]` | `size`     | `ℕ`（`u64`）     | **元素个数** | 自 `data` 起可容纳的元素个数（容量）                                  | 由 `T*` coerce 降级（§2.7），`size` 保留；访问约束见 `in_bounds` |
| `T&`  | `data`     | `Addr`（`T*`）   | 字节         | 被引用对象负载区地址（`index` 已折叠进 `data`）                       | 由 `T*`/`T[]` coerce 降级（§2.7），锁继承源指针                  |
| `T&`  | `lock_ptr` | `Addr`（`u64*`） | 无（地址）   | 同 `T*`：锁槽地址（锁继承源指针）                                     | 由 `T*`/`T[]` coerce 降级（§2.7）                                |
| `T&`  | `key`      | `Key`（`u64`）   | 无（键值）   | 同 `T*`：时序键（锁继承源指针）                                       | 由 `T*`/`T[]` coerce 降级（§2.7）                                |

`T&` 恒指向单个元素，无 `index`/`size` 字段，故无越界概念（定义 12 对 `T&` 退化，见下）。`T[]` 的 `size` 与派生 `T*` 的 `size` 一致，界检查在派生 `T*` 上执行（§2.7、定义 12）。

**定义 12（空间有效性谓词 `in_bounds`）**：对跨度 $n$ 个元素（$n \geq 0$）的访问，`T*`/`T[]` 指针 $p$ 满足：
$$\text{in\_bounds}(p, n) \iff 0 \le p.\text{index} \;\wedge\; p.\text{index} + n \le p.\text{size}$$
该谓词为**全访问检查**：要求整个访问区间 $[\text{index}, \text{index} + n)$ 落在对象内，而非仅检查访问起点。单元素读写即 $n = 1$。**对 `T&` 引用退化**：`T&` 无 `index`/`size` 字段（定义 11），恒指向单个元素、无越界概念，故 $\text{in\_bounds}(r, n)$ 对任意 $n \ge 0$ **恒为真**——`T&` 的访问不需空间检查，仅需时序检查 `live`（定义 14、规则 3.2.1-3.2.2 分级前提）。`T[]` 切片的界检查在派生 `T*` 上执行（§2.7：切片访问经 `ptr()` 派生 `T*` 后按本定义检查）。
...
第 3 章每条访问规则的正常前提即 `safe_access`；前提不满足则转移到 `trap`。**分级实例化**：对 `T*`/`T[]` 指针，`safe_access = live ∧ in_bounds`（全检查）；对 `T&` 引用，`in_bounds` 恒真（定义 12 退化），故 $\text{safe\_access}(r, n) \iff \text{live}(r)$——引用访问仅需时序检查（《编译器实现》§7.1 CheckRefAccess，仅发射 `live`）。
...
分配结果恒为 `T*`（5 字段，定义 11）；需要 `T[]`/`T&` 时经 coerce 降级派生（§2.7）。
...
**定义 15（帧内取址）**：指向当前帧锁 $\langle e_f, k_f \rangle$，取址结果恒为 `T*`（5 字段，定义 11）：
...
取址不产生 `T[]`/`T&`；`T&` 引用由 `T*`/`T[]` 经 coerce 降级派生（§2.7），锁字段继承，故其 `live` 语义与母指针完全一致——`T&` 恒为单元素、无越界概念（定义 12 退化），仅需时序检查。
...
表示层整体原则：胖指针按级承载操作——`T*`（5 字段）承载全部指针操作，`T[]`/`T&`（降级视图）只承载其允许的子集。**`index` 是 `T*` 中唯一的可变派生分量**（算术/重锚定/折算都只改 `index` 或建立新锚），`data`/`lock_ptr`/`key`/`size` 在指针整个生命周期内不变；「同对象指针共享锁槽与键」是表示层的结构性事实，派生指针（含降级的 `T[]`/`T&`）的时序失效与母指针同步。
...
- **指针减法 $p_1 - p_2$（仅 `T*`）**：要求 $\text{data}$ 相等（同对象），结果为元素差 $\text{index}_1 - \text{index}_2$。
- **比较（仅 `T*`）**：同对象比较基于 `index`；`live` 不作为比较前提。
- **解引用与索引**：`*p`（`T*`）访问前提为 `safe_access`；`*r`（`T&`）前提为 `safe_access` 的 `T&` 实例化（仅 `live`）。

**coerce 链（降级）**：`T* → T[] → T&` 三级降级由手动标注触发（《编译器实现》§8.1 规则 8.1.13-8.1.15），折叠规则：

- `T* → T[]`：`index` 折叠进 `data`——$\text{data}' = \text{data} + \text{index}\cdot|T|$，删 `index` 字段，`size` 保留（容量不变）；
...
**规则 3.2.1（解引用读 `*p`）**（→ CFG `Load`）。前提：`safe_access(p, 1)`。动作：读取 `footprint_T(p, 1)` 处存储的 `T` 值。结果：μ 不变，读得值参与后续计算。若前提不满足则 trap（悬垂读、越界读、one-past-end 读均在此 trap）。**分级前提**：`p` 为 `T*`/`T[]` 派生指针时 `safe_access = live ∧ in_bounds`；`p` 为 `T&` 引用时 `in_bounds` 恒真（定义 12 退化），前提退化为 `live(r)`——CFG 层对 `T&` 插 `CheckRefAccess`（仅 `live`），对 `T*` 插 `CheckSafeAccess`（《编译器实现》§7.1）。

**规则 3.2.2（解引用写 `*p = v`）**（→ CFG `Store`）。前提：`safe_access(p, 1)`。动作：将 `v` 写入 `footprint_T(p, 1)` 全部位置。结果：μ 在 `footprint_T(p, 1)` 上更新为 `v`。若前提不满足则 trap。**分级前提**：同规则 3.2.1。
...
**陈述规范**：不变量与引理以全称闭式给出，自由变量全部显式量化。谓词带时间下标 $t$ 时，表示按第 $t$ 步后配置 $\langle \mu_t, C_t \rangle$ 的锁槽内容求值（`live`/`in_bounds` 的定义 8/12 均为 $\mu$ 读），是既谓词的实例化而非新谓词。

**定义 20（活动锁槽 `Active` 与当前键 `key_of`）**：锁槽地址 $e$ 在时刻 $t$ 活动，当且仅当其 $\mu$ 内容为活动锁编码：

$$\mathrm{Active}_t \triangleq \{ e \in \mathrm{Addr} \mid \mu_t\langle e \rangle = k \ \text{对某}\ k \in \mathrm{Key} \}$$

$\mathrm{key\_of}_t(e)$：当 $\mu_t\langle e \rangle = k$ 时等于 $k$；当 $\mu_t\langle e \rangle = \text{SENTINEL}$ 或为任意非键值时无定义。

**定义 21（运行轨迹 `Trace`）**：轨迹 $\sigma$ 是配置的有限或无限序列 $\sigma_0 \longrightarrow \sigma_1 \longrightarrow \cdots$，其中 $\sigma_0 = \langle \mu_0, C_0 \rangle$ 为初始配置（$\mathrm{dom}(\mu_0) = \emptyset$），每步按第 3 章规则转移，直至 `trap` 或程序终止。$\mu_t$ 为 $\sigma_t$ 的存储分量。$\sigma \in \mathrm{Trace}(P)$ 表示 $P$ 的一次合法运行。

**定义 22（访问事件与成功判定 `ok`）**：轨迹第 $t$ 步对指针 $p$ 执行一次读或写（跨度 $n \ge 1$）记事件 $\mathrm{acc}_t(p, n)$；对 $p$ 执行释放记事件 $\mathrm{del}_t(p)$。事件成功 $\mathrm{ok}(\cdot)$ 当且仅当该步按第 3 章规则正常转移。

**定义 23（对象动态类型 `dyn_type` 与静态类型 `static_type`）**：$\mathrm{dyn\_type}_t(a)$ 为时刻 $t$ 锚定于地址 $a$ 的对象的动态类型，由建立该对象的分配（`dyn T` / `dyn T[n]`）或取址的静态类型唯一决定；$a \notin \mathrm{dom}(\mu_t)$ 时无定义。$\mathrm{static\_type}(p)$ 为指针表达式 $p$ 的编译期静态类型。
...
**论证梗概**：对 $t$ 归纳。基步 $t = 0$ 无访问事件；归纳步按第 $t$ 步规则分情形：读/写规则的正常前提均含 `safe_access(p, ·)`，而 `safe_access` 蕴含 `in_bounds`，故成功访问处 `in_bounds` 成立；指针算术规则要求结果良构，良构性沿分配、取址保持。对 O-1 的三个子义务：① 规则前提枚举是语法可见的，两条规则（3.2.1-3.2.2）的正常前提逐字含 `safe_access(p, n)`，无隐藏访问路径；② 良构性保持沿归纳步逐一验证；③ 无回绕由 §2.4 实现约定承担。

**义务 O-1**：证明每类成功访问的规则前提含 `in_bounds`；证明良构性沿指针算术/取址保持；证明 `index + n` 无回绕。
...
**论证梗概**：正向侧直接来自规则结构：读/写的正常前提含 `safe_access`（蕴含 `live`），释放前提含 `live`，故成功即在访问时刻 `live` 成立。负向侧（作废永久性）为实质内容，由『锁槽内容与旧键失配』统一闭合：锁槽重激活只获新键（L-NOKEY 实例一/实例二）、它用为任意值或哨兵（值失配不等式）——哨兵与普通数据就碰撞概率无区别，统一由 $v \ne k$ 闭合。

**义务 O-2**：O-2a 验证读/写/释放规则的正常前提均含 `live`；O-2b 以 L-NOKEY（及统一值失配）证明作废永久性：对任何作废对象，其全部派生指针此后 `live` 恒为假。
...
**义务 O-3**（时序安全的基石）：O-3a 证明 (i)（定义 10 的组合逻辑）；O-3b 证明锁槽写入规则枚举完备，凡 $\mu\langle e \rangle := k$ 者均以 `k ← Gen()` 产生；O-3c 由 (i) 推出 (ii) 与 (iii)。**建模备注**：CSPRNG 路径下形式论证在概率意义下成立，以可忽略概率违反。

### 4.5 锁槽键不复用引理 L-NOKEY

**引理 L-NOKEY（锁槽键不复用/重激活只获新键）**：设 L-KEY 成立。对任意锁槽 $e$（堆或帧）、任意两次锁槽**被写入**活动键的时刻 $t_1 < t_2$ 与任意 $k_1, k_2$：
$$\mu_{t_1}\langle e \rangle = k_1 \wedge \mu_{t_2}\langle e \rangle = k_2 \Rightarrow k_1 \ne k_2$$

**推论（旧指针永久失效）**：携带 $\langle e, k_1 \rangle$ 的指针 $p$ 在 $t_2$ 及此后 $\mathrm{live}$ 恒为假：
$$\forall s \ge t_2.\ \neg \mathrm{live}_s(p)$$

**论证梗概**：

- 实例一（堆重激活）：$t_2$ 由分配规则重新激活，写入的 $k_2$ 是 `Gen` 的新调用输出；由 L-KEY(iii)，$k_2$ 与 $e$ 的全部历史键不同，特别 $k_2 \ne k_1$。此后堆锁槽取值三情形：活动新键（L-KEY 保证 $\ne k_1$）、复用作另一对象锁槽（键亦来自 `Gen`、$\ne k_1$）、哨兵或任意非键值（统一值失配 $v \ne k$；前提一：经检查写不可达锁槽，由布局推导为 L-UNREACH，§4.8；前提二：定向写排除，§1.2）——故 $\mathrm{live}(p)$ 恒假。作废自哨兵写入（中间时刻 $t_1'$）即开始，该 $(t_1', t_2)$ 窗口由值失配闭合（哨兵亦为非键值）。
- 实例二（栈帧 re-key）：帧进入规则 3.7.1 每次执行 re-key：$k_f \leftarrow \mathrm{Gen}()$，由 L-KEY(i) 两轮进入键不同（即使同一帧递归/循环复用同一锁槽）；帧退出先写哨兵（3.7.2 动作①）保证两轮进入间 $e_f$ 必经过非活动键值，上轮遗留指针携带 $k_1$ 与 $t_2$ 后帧锁键 $k_2$ 不匹配；re-key 每次独立调用，键隔离不依赖 enter/exit 配对完整性。

**义务 O-4**（L-NOKEY 的证明义务，T1 负向侧，含子义务 O-4a 堆实例/O-4b 栈实例）：O-4a 以 L-KEY 证明堆锁槽复用只获新键、旧指针键永不重新匹配，证明 $e$ 重新激活后只存贮活动新键或作废值（含哨兵与任意非键值）——由统一值失配 $v \ne k$ 闭合（前提一 L-UNREACH、前提二 定向写排除）；O-4b 以 L-KEY 证明同一帧锁槽两轮帧进入的键不同、帧退出先写哨兵、上轮指针此后不匹配（规则 3.7.1-3.7.2 枚举）。

**义务 O-4 的验证次序**：先枚举锁槽（$\mu$ 内容）的全部写入点（分配 3.6.1、释放 3.6.2、帧进出 3.7.1-3.7.2），确认其中写活动键者必先 `Gen()`；再对每条重激活路径（堆：分配重激活；栈：帧进入 re-key）应用 L-KEY(iii) 得新键 ≠ 全部历史键；次序固定后，O-4 可机械执行，无开放性反例分支。
...
**为什么运行时论证不需要它**：运行时检查（`in_bounds` 定义 12、`live` 定义 8）是类型无关的——检查对象是分级元组的区间与锁键（`T*` 5 元组、`T[]` 4 元组、`T&` 3 元组，定义 11），不引用 `dyn_type`/`static_type`；S1（§4.2）、T1（§4.3）与 L-KEY 家族（§4.4-§4.5）的陈述与证明均不依赖类型同一性。类型混淆的潜在安全后果——`delete` 超量释放——被整块交还语义结构性排除：释放范围 = 分配器元数据决定的整块（规则 3.6.2 动作②），与 `p.size·|T|` 无关；用户代码无 `bitcast`（受限操作检查），标准库内部 `bitcast` 保持 `data`/`lock_ptr` 不变，混淆指针的 `delete` 仍交还其所属整块。叠加语法级排除 + 标准库审计 + `delete` 仅允许原始指针（`is_raw` 原始分配指针校验，§2.5）的封闭，故定理 5.1（§5.1）无须把类型同一性列为前件或结论。

### 4.7 引理间依赖与第 5 章接口

依赖关系：L-KEY（O-3）→ T1 负向侧（O-2b，统一值失配）；L-KEY（O-3）→ L-NOKEY（O-4）→ T1 负向侧；S1（O-1）→ L-UNREACH（O-5）→ L-NOKEY（O-4 前提一）；S1 本身独立于时序链；类型混淆排除不依赖本章引理（可信基边界，§4.6）。
...
| L-NOKEY     | O-4（O-4a 堆 / O-4b 栈）  | T1 负向侧（堆复用期+栈复用期，含它用值失配） |
| L-UNREACH   | O-5                       | 值失配前提一：经检查写不可达锁槽（布局推论） |

第 5 章主可靠性定理将把「trap 之外的一切内存访问均安全」归结为：S1 排除越界（G1），T1 排除 UAF、双释放与栈悬垂（G2），类型混淆由可信基边界排除（§4.6，G4）；L-KEY 家族保障 T1 的作废永久性，即「检查当时通过、此后失效不可逆」的时序封闭性。本组不变量仅在 §1.3 in 类范围内成立；并发、FFI 等 out 情形不适用。**依赖图核对**：依赖链上不存在环：S1 只依赖规则前提枚举与良构性保持，不依赖任何时序引理；L-UNREACH 是 S1 + 块布局（定义 7）的直接推论，随 S1 链推进、为 O-4 前提一提供依据，不依赖时序引理；L-KEY 依赖定义 10 与锁槽写入规则，独立于 S1；L-NOKEY 以 L-KEY 为前提，O-4a（堆侧）与 O-4b（栈侧）子义务并行、互不依赖，堆侧另以 L-UNREACH（O-5）为值失配前提一的依据；T1 正向侧（O-2a）依赖规则文本，负向侧（O-2b）依赖 L-NOKEY（统一值失配）；类型混淆排除（§4.6）不依赖本章任何引理。因此证明义务可按 S1（含 L-UNREACH）、L-KEY 两条链并行推进，T1 在两条链完成后收口，无循环依赖。
...
本章不做新的形式证明，而是把第 4 章的不变量与引理（S1/T1/L-KEY/L-NOKEY/L-UNREACH）及其证明义务（O-1 至 O-5，含子义务）组织为一份可执行的可靠性论证：§5.1 给出单句主可靠性定理，§5.2 给出义务↔引理映射表，§5.4 说明拼接结构，§5.5 显式声明论证范围与未承诺事项。论证层级为「梗概级」（S&P/CCS/USENIX 会议的严谨度），未验证的证明义务以清单形式留给实现与定理证明器工作，PLDI 级完整证明不在本章范围。本章只引用第 2-4 章符号（定义 1-23）与规则，不定义任何新记号。
...
- **G4（类型）**：不进入定理 5.1——类型混淆由可信基边界排除（§4.6：用户代码语法级无 `bitcast`/`from_raw_parts`，标准库审计），定理 5.1 不承诺「无类型混淆访问」；该承诺由受限操作检查 + 标准库审计承担（《编译器实现》§8.3）。

### 5.2 证明义务清单与义务↔引理映射

表 4 把第 4 章定义的证明义务（§4.1 编号，§4.2-§4.8 逐条陈述）组织为义务↔引理映射：行 = O-1 至 O-5（含子义务），列 = 支撑引理/不变量、对应安全目标、论证梗概要点。依赖关系沿用 §4.7。
...
| O-2b     | T1 负向侧；L-NOKEY（§4.5）   | G2（时序）                | 作废永久性：锁槽内容与旧键失配统一（哨兵/新键/任意值，统一值失配）    |
...
| O-4      | L-NOKEY（O-4a 堆 / O-4b 栈） | G2（T1 堆侧+栈侧）        | 堆锁槽复用只获新键、同锁槽两轮帧进入键不同，旧指针键永不重新匹配      |
| O-5      | L-UNREACH（§4.8）            | 值失配前提一（布局推论）  | 分配锚定 data = b+H；in_bounds 足迹 ⊆ 负载区；锁头区与负载区不交      |

表中的「支撑」列为义务完成后可得的引理结论；义务未验证即主定理前件不成立，故本表同时是「未验证证明义务」的清单（§5.5）。**表 4 的读法**：O-1 与 O-2a 是纯枚举义务，可由代码审查直接勾销；O-3a 是定义 10 的组合逻辑；O-3b/O-3c 依赖写入规则枚举与 (i)，属轻量归纳；O-4 是 L-KEY 的直接应用；O-5 是 S1 + 块布局的直接推论（验证锚定与布局，无时序内容）；类型混淆排除依赖《编译器实现》第 8 章受限操作检查与审计准则（§4.6），不在表 4 义务范围内。义务间的传递依赖是 O-3 → O-2b（统一值失配）、O-3 → O-4 → O-2b、O-1 → O-5 → O-4（值失配前提一），其余并行，与 §4.7 依赖图一致。

### 5.4 论证拼接结构

定理 5.1 由两条不变量（S1、T1）与 L-KEY 家族及锁槽可达性引理（L-UNREACH）支撑拼接而成，与 §4.7 依赖表一致：

1. **S1（O-1）排除越界 → G1**。
2. **T1（O-2a ∧ O-2b）排除时序错误 → G2**。
3. **类型混淆排除（§4.6，可信基边界）**。
4. **L-KEY 家族支撑 T1 的作废永久性**。
5. **锁槽可达性（O-5）补足复用安全的地基**。
...
| 无栈悬垂访问    | T1（O-2b，L-NOKEY 栈实例）→ G2 | 帧退出后对 `&x` 的访问        | 旧键 $k_1$ 与当前帧锁槽 $k_2$ 不匹配，trap |
| 无回绕误判      | O-1 无回绕子义务 → G3          | `p + 2^{64}-1`                | 数学整数求值，`in_bounds` 恒不成立，trap   |

上表每行把定理 5.1 的负面结论落到一个可构造的访问事件上：事件在规则前提处失败即 trap（§2.1 吸收态），不存在「检查通过但访问越界/悬垂」的余项；类型混淆不在上表（由可信基边界排除，§4.6，无运行时事件，也不是定理 5.1 的承诺）。
...
- **标准库可信基假设**：定理 5.1 前件要求用户代码不含受限操作（`bitcast`/`from_raw_parts` 等，受限操作检查，语法级）；标准库内部受限操作的元数据转化由《编译器实现》§8.3 承担（实现篇留白），属可信基假设（与分配器行为、无回绕实现约定同构）——审计失守不构成机制反例。该假设是 G4 排除的依据（§1.3 类型行 out、§4.6）。
- **前置条件显式**：定理 5.1 的五条前件（① 类型约束、② 良构性、③ 块布局、④ 经检查写不可达锁槽（L-UNREACH）、⑤ 定向写排除）均不自动成立——类型检查不通过或出现非良构操作的程序不在结论范围；③④ 由 §4.8 的义务 O-5 逐项验证，⑤ 是 §1.2 攻击者模型（写须通过检查）的直接推论。out 行失效的前提是 §1.2 攻击者假设被放宽（例如攻击者获得任意写前置），故 out 行失效不构成 in 类论证的反例。
...
- **单调键生成 `Gen`（定义 10）与键唯一性**：任意两次 `Gen` 调用输出不同，键序列唯一性是 L-KEY（§4.4）的前提，进而支撑作废永久性（L-NOKEY）与锁槽复用值失配论证（§2.3「锁槽复用」集中说明）。
...
- **CFG 层 + 内联分级字段表示（《编译器实现》§7.4 方案 A，tiered-pointers 三级 40/32/24B）与元数据防伪**：检查插入点即第 3 章规则前提落地；元数据不可伪造依赖 §1.2 无任意写前置假设（§1.3 out 行显式声明）。
