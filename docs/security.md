...
注记：量词域为「被写入活动键的时刻」（即重激活/帧进入的写入时刻），**不是**「锁槽内容为活动键的时刻」——后者是状态谓词，同一生命周期内取两个时刻会违反结论（锁槽持续持键 $k$ 时 $k_1 = k_2 = k$）；原 L-REKEY 以「帧进入时刻」、原 L-NOREUSE 以 SENTINEL 中间合取 $t_1'$ 保证量词域，统一后由本注记与论证梗概 ①（作废自哨兵写入即开始）共同承接。每次写入活动键必先 `Gen()`（L-KEY 前提），故两次写入键不同。

**推论（旧指针永久失效）**：携带 $\langle e, k_1 \rangle$ 的指针 $p$ 在 $t_2$ 及此后 $\mathrm{live}$ 恒为假：
$$\forall s \ge t_2.\ \neg \mathrm{live}_s(p)$$

**论证梗概**：① 实例一（堆重激活）：$t_2$ 由分配规则（3.6.1）重新激活，写入的 $k_2$ 是 `Gen` 的新调用输出；由 L-KEY(iii)，$k_2$ 与 $e$ 的全部历史键不同，特别 $k_2 \ne k_1$。此后堆锁槽取值三情形：活动新键（L-KEY 保证 $\ne k_1$）、复用作另一对象锁槽（键亦来自 `Gen`、$\ne k_1$）、哨兵或任意非键值（统一值失配 $v \ne k$；前提一：经检查写不可达锁槽，由布局推导为 L-UNREACH，§4.8；前提二：定向写排除，§1.2）——故 $\mathrm{live}(p)$ 恒假。作废自哨兵写入（中间时刻 $t_1'$）即开始，该 $(t_1', t_2)$ 窗口由值失配闭合（哨兵亦为非键值）。② 实例二（栈帧 re-key）：帧进入规则 3.7.1 每次执行 re-key：$k_f \leftarrow \mathrm{Gen}()$，由 L-KEY(i) 两轮进入键不同（即使同一帧递归/循环复用同一锁槽）；帧退出先写哨兵（3.7.2 动作①）保证两轮进入间 $e_f$ 必经过非活动键值，上轮遗留指针携带 $k_1$ 与 $t_2$ 后帧锁键 $k_2$ 不匹配；re-key 每次独立调用，键隔离不依赖 enter/exit 配对完整性。③ 键位编码注记：堆键最高位 1、栈键最高位 0，属实现细节，不进入核心陈述。
...
**为什么运行时论证不需要它**：运行时检查（`in_bounds` 定义 12、`live` 定义 8）是类型无关的——检查对象是 5 元组的区间与锁键，不引用 `dyn_type`/`static_type`；S1（§4.2）、T1（§4.3）与 L-KEY 家族（§4.4-§4.5）的陈述与证明均不依赖类型同一性。类型混淆的潜在安全后果——`delete` 超量释放——被整块交还语义结构性排除：释放范围 = 分配器元数据决定的整块（规则 3.6.2 动作②），与 `p.size·|T|` 无关；用户代码无 `bitcast`（受限操作检查），标准库内部 `bitcast` 保持 `data`/`lock_ptr` 不变，混淆指针的 `delete` 仍交还其所属整块。叠加语法级排除 + 标准库审计 + `delete` 仅允许原始指针（`is_raw` 原始分配指针校验，§2.5）的封闭，故定理 5.1（§5.1）无须把类型同一性列为前件或结论。
