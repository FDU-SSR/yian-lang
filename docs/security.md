...
**锁槽复用（lock slot reuse）**：锁槽是块头/帧首的物理内存，随对象分配/帧进入而生、随释放/帧退出而空出。本设计的时序安全中「锁槽可复用，但键不可复用」。锁槽位置在块释放/帧退出后脱离守卫职责，可被分配器复用。无论锁槽内容如何——重激活写新键 $k'$（规则 3.6.1/3.7.1 先 `Gen()`）、被它用为普通数据或哨兵值——时序检查 `live(p)`（定义 8）比较锁槽物理值 $v = \mu\langle p.\text{lock\_ptr} \rangle$ 与指针键 $p.\text{key}$：哨兵值与复用后的普通数据，就与 key 碰撞的概率而言无区别（可忽略概率），统一由『锁槽内容与旧键失配』（$v \ne k$）闭合；重激活路径由 L-KEY/L-NOREUSE 保证新键 ≠ 历史键。
...
**规则 3.7.1（帧进入 `enter f`）**（帧级，CFG 无对应节点）。前提：无。动作（§2.6）：帧首锁槽 `e_f` 就位（帧首字，定义 7），`k_f ← Gen()`（**re-key：每次进入生成新键**，定义 10 保证与上次进入不同）；帧锁槽写键 `μ⟨e_f⟩ := k_f`（栈键最高位 0）；当前帧锁 := `⟨e_f, k_f⟩`；帧栈块区间并入 `dom(μ)`。结果：帧内 `&x` 等取址以 `⟨e_f, k_f⟩` 为帧锁（定义 15）。

**规则 3.7.2（帧退出 `exit f`）**（帧级，CFG 无对应节点）。前提：无。动作（§2.6）：帧锁槽写哨兵 `μ⟨e_f⟩ := SENTINEL`；帧栈块区间自 `dom(μ)` 撤销（栈内存恒映射，帧锁槽随即由复用方支配；复用后由 L-REKEY/值失配闭合）。结果：帧内产生的全部指针此后 `live` 恒为假；对已退出帧地址的任何后续访问在访问规则处 trap（栈悬垂防护）。

### 3.8 bitcast

**规则 3.8.1（`bitcast`）**（→ CFG `Cast`）。前提：《编译器实现》第 8 章类型约束成立——`bitcast` 仅限标准库（受限操作检查，标准库外编译期拒绝），标准库内部 pointee 由 `T` 改为 `U` 时按审计准则满足布局兼容（`p.index·|T|` 与 `p.size·|T|` 均被 `|U|` 整除，不破坏对齐，《编译器实现》§8.3）；不访问内存，无 `live`/`in_bounds` 检查。动作：5 字段中 `data`/`lock_ptr`/`key` 不变，按字节长度重折算元素单位：`size' := ⌊(p.size·|T|)/|U|⌋`，`index' := ⌊(p.index·|T|)/|U|⌋`。结果：`⟨p.data, p.lock_ptr, p.key, index', size'⟩`。若前提不成立则为非良构程序（编译期拒绝）；反向转换（`|U|` 不整除字节跨度）折算非整即前提不成立，亦编译期拒绝。

## 4. 不变量与引理

本章把第 2-3 章表示与规则中的安全性要求升格为可陈述、可引用的不变量与引理。第 5 章主可靠性定理以 S1、T1 两条不变量为直接前提（类型混淆排除为可信基边界，§4.7），以 L-KEY、L-NOREUSE、L-REKEY、L-UNREACH 为中间结论支撑 T1 的作废永久性一侧。每条 = 陈述（闭公式）+ 论证梗概 + 证明义务，不给出 PLDI 级完整证明（明确排除）；论证范围限于 §1.3 的 in 类程序（单线程、不跨 FFI、攻击者无任意写前置，`bitcast` 等受限操作由语法级禁令排除于用户代码、标准库为审计可信基）。符号全部沿用第 2-3 章（定义 1-19），不重复定义；新定义自编号 20 起。
...
**论证梗概**：对 $t$ 归纳。基步 $t = 0$ 无访问事件；归纳步按第 $t$ 步规则分情形：读/写规则（3.2.1-3.2.2）的正常前提均含 `safe_access(p, ·)`，而 `safe_access` 蕴含 `in_bounds`（定义 14），故成功访问处 `in_bounds` 成立（关键步骤为「成功 ⟹ 规则前提满足 ⟹ 前提含界检查」）；指针算术规则（3.3.1-3.3.2）要求结果良构（定义 13），良构性沿分配、取址（`index := 0`）与 `bitcast` 折算（§3.8）保持；§2.4 无回绕约定保证 `index + n` 在数学整数上精确求值（类 4，§1.1）。对 O-1 的三个子义务：① 规则前提枚举是语法可见的，两条规则（3.2.1-3.2.2）的正常前提逐字含 `safe_access(p, n)`，无隐藏访问路径；② 良构性保持沿归纳步逐一验证（分配返回 `index = 0`、取址重置 `index = 0`、算术前提显式要求 `0 ≤ index + n ≤ size`、`bitcast` 按 `⌊·⌋` 取整不越界）；③ 无回绕由 §2.4 实现约定承担。one-past-end（`index = size`）在良构上允许（定义 13），但一切 $n \geq 1$ 访问在 `in_bounds` 处失败，不构成 S1 反例。

**义务 O-1**：证明每类成功访问的规则前提含 `in_bounds`（§3.2-§3.8 规则枚举）；证明良构性沿指针算术/取址/`bitcast` 保持；证明 `index + n` 无回绕（§2.4 约定）。该义务是第 5 章「无越界访问」结论的直接前提。
