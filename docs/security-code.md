...
**Cast**（`ir.py:122`）。现状：pointer→pointer 为单条 `bitcast`。变化：标准库内部 `bitcast` 的元数据转化（`index`/`size` 重折算）**暂留白**——后续将对标准库删减，待删减后确定（第 11 章未来工作）；理论论证不涉及（标准库为可信基黑盒，《形式化论证》§2.7）。检查插入：无运行时检查（编译期受限操作检查：`bitcast` 仅限标准库，第 8 章承担）。结论：**需改造**（表示重折算；编译期前提移交第 8 章）。 **风险评估**：真风险点（标准库内部 bitcast 元数据转化留白，第 11 章）；风险来自标准库字节级重解释与结构视图转换依赖，详见 §8.3 扩展；风险评估=**留白（维持既有元数据转化留白）**，节点结论仍为既有「需改造」（不覆盖）
**SizeOf**（`ir.py:135`）。现状：编译期常量折叠（`sizeof_const`），返回 $|T|$ 字节。变化：节点语义不变；但布局表对 `PointerType` 的映射从 8B 变 40B（`types.py:229-230`），含指针的聚合类型尺寸随之变化。检查插入：无。结论：**直接**（节点免改，全局布局表受影响）。补充：`sizeof(T*)` 在胖指针表示下返回 40，故 `dyn T*[n]` 等指针数组的分配字节数、$\text{data} + \text{index} \cdot |T|$ 折算中的 $|T|$ 均自动采用新布局；跨类型折算细节见 §8.3（标准库内部 bitcast 元数据转化留白，第 11 章），SizeOf 本身不引入任何检查点。
...
3. **指针比较**：`icmp` 直比改为字段比较（规则 3.4.1-3.4.2）；LLVM 无聚合 `icmp`，比较由 1 条变为「提取字段 + 2-3 条 + 前提检查」，Binary 须新增指针分支。序比较还需在比较前查 `data` 相等（规则 3.4.1 前提）；相等比较按 `(data, index)` 二元组（规则 3.4.2），与序比较分支结构不同，Binary 指针分支内须再分序/等两路。
...
**规则 8.1.4（空指针字面量）**：对任意类型 $T$ 有 $\Gamma \vdash \texttt{null} : T^{*}$（`coerce` 中的无条件指针隐转之一，另一条为数组退化 $T[m]^{*} \to T^{*}$（8.1.3），`expr_checker.py:341-345`）。
...
**标准库审计准则**：标准库内部 `bitcast` 的元数据转化**暂留白**——后续将对标准库删减，待删减后确定（第 11 章未来工作）；理论论证把标准库作为可信基黑盒，不涉及此处细节。当前标准库使用 `bitcast` 的 10 个文件（`lib/core/hash.an`、`lib/core/slice.an`、`lib/core/array.an`、`lib/core/str.an`、`lib/num/*.an`）与使用 `from_raw_parts` 的 4 个文件（`lib/core/slice.an`、`lib/core/vec.an`、`lib/core/env.an`、`lib/core/array.an`）清单作为留白期间的事实记录保留。

### 受限依赖风险评估

> 既有清单数量已修订，以下为 grep 核实的完整计数：bitcast 14 文件 23 处、from_raw_parts 17 文件 34 处、nullptr 1 处。

本子段把标准库对受限操作（`bitcast`/`from_raw_parts`/`nullptr`）的依赖逐点分类，按「可移除 / 需保留 / 需改造」三档裁决，并给出胖指针化后的替代路径。裁决只记录评估结论，移除执行属第 11 章未来工作（§11.2）。
...
**移除可行性结论**：可移除项（结构转换 bitcast + from_raw_parts 切片视图站点）移除后，§7.1 Cast 留白（L22）的论证缺口收窄到「字节级哈希」一类；该类的消除路径 = memcpy 走字节指针（保留为受限名）或编译期内建字节视图（intrinsic），见 §11.2。
...
- **标准库受限依赖移除**：按 §8.3 受限依赖风险评估的三档裁决，移除可移除项——结构转换 `bitcast`（`lib/core/array.an`/`slice.an`/`str.an` 的 T[N]↔T*、SliceStruct↔T[]、u8[]↔str 视图，胖指针化后由数组退化与聚合构造承担）与 `from_raw_parts` 切片视图站点（`slice.an`/`array.an`/`vec.an`/`pointer.an`，由 5 字段聚合构造直接覆盖）；字节级哈希一类（`lib/num/*.an` 与 `hash.an` 的 memcpy 字节视图）收窄为 memcpy 保留为受限名或编译期内建字节视图（intrinsic）。动机：闭合 §7.1 Cast 留白（标准库内部 `bitcast` 元数据转化）的论证缺口；env.an FFI 裸指针站点属 out 边界，不在移除范围。
