...
**Malloc**（`ir.py:67`）。现状：元素数换算字节数 + `malloc` intrinsic + bitcast，pointee 为 ZST 时返回 undef。变化：按规则 3.6.1：块头锁槽写入键 `k ← Gen()`（定义 10）、`μ(e) := ⟨heap, k⟩`，返回 `⟨b, e, k, 0, n⟩`，`lock_ptr` 指向块头锁槽；与块头布局（定义 7）交互。检查插入：无 trap 前提（分配恒可执行）；插入块头锁槽写键。结论：**需改造**（块头布局 + 写键）。补充：翻译后在块头写键 + 5 字段聚合构造 + 写分配点记录（`block_origin`/`block_bytes`，供 `Delete`/`realloc` 的 `is_origin` 校验读取）；ZST 情形返回合法 `⟨b, e, k, 0, 0⟩`，`size = 0`，一切访问在 `in_bounds` 失败（规则 3.6.1）。
**Delete**（`ir.py:100`）。现状：bitcast 为 `i8*` 调 `free`，ZST 指针释放为 no-op。变化：前提 `is_heap(p) ∧ live(p) ∧ p.index = 0 ∧ is_origin(p)`（规则 3.6.2；前提求值顺序 `live`/`is_heap` 先于或并列 `index = 0`，且对 `lock_ptr = 0` 短路为假——`delete(null)` 确定性 trap；`is_origin` 定义见《形式化论证》§2.5），动作先 `μ(p.lock_ptr) := SENTINEL` 再块入隔离（内存保持可读、用户数据区逻辑上撤销、锁槽区留在 `dom(μ)`，隔离假设《形式化论证》§2.2）。检查插入：四前提检查（锁槽 kind 判定 `is_heap`，定义 9；键比较 `live`；`index = 0`；`is_origin` 读块头分配点记录）。结论：**需改造**（四检查 + 锁槽写哨兵）。补充：`is_heap` 与 `live` 对同一锁槽只做一次寻址、键比较共享读取结果，`is_origin` 另读块头分配点记录；释放范围按 `p.size·|T|` 计算；**`is_origin(p)`**：`p.data = block_origin(p.lock_ptr) ∧ p.size·|T| = block_bytes(p.lock_ptr)`（`|T|` 为 delete 处 pointee 字节大小，Malloc 写入块头、Delete 读取校验）——重锚定子对象指针（`&s.field`、`&arr[i]`）`delete` 视为错误操作（trap），即使 `index = 0`；不再依赖根追踪（§8.4 降级为诊断用途）。
...
6. **null 编码**：NullptrLiteral 选 `⟨0, 0, 0, 0, 0⟩` 编码后，对 null 的访问经 `live`/`is_heap` 短路（`lock_ptr = 0` → 假，定义 8/9）trap——不依赖「`lock_ptr = 0 ∉ dom(μ)`」的论证；须与第 8 章「指针只由良构操作产生」闭合（§8.1 值域闭合已穷尽指针产生途径：`bitcast` 与无检查的 `from_raw_parts` 均被受限操作检查禁于标准库外，非规范 5 元组无法由良构定型产生，故 `⟨0,0,0,0,0⟩` 之外不出现可逃逸的编码）。
...
本章把「类型系统约束也是安全机制的一部分」落实为可陈述的形式化：§8.1 给出指针定型规则（判定式/规则式），§8.2 定义对象类型同一性与分配点类型标注，§8.3 给出受限操作检查（bitcast 仅限标准库），§8.4 给出根/来源追踪设计（规定式），§8.5 给出类型约束与运行时检查的分工表及完备性论证。类型混淆排除（降级为可信基边界，不占义务编号，《形式化论证》§4.7）的编译期侧在此承接；第 7 章 §7.6 风险 6（null 编码）要求的「指针只由良构操作产生」亦在本章定型规则中闭合。符号全部沿用《形式化论证》第 2-5 章（定义 1-23、规则 3.2.1-3.10.1），不重新定义；新定义自编号 24 起。论证范围同《形式化论证》§1.3 的 in 类；本章只对指针值定型，不引入借用/生命周期分析，亦不重构语言其余类型系统。
...
**隔离假设（out，实现选择边界）**：本机制要求释放后内存保持映射可读（不 `munmap`，《形式化论证》§2.2）：释放协议写 SENTINEL 后块入隔离、内存不归还 OS，使隔离期内锁槽读得确定性哨兵。若实现选择把释放内存归还操作系统（`munmap`），读已释放锁槽由 trap 退化为段错误，属实现选择边界（论证范围声明）。隔离期结束、块被复用后锁槽物理位置留在 $\mathrm{dom}(\mu)$、脱离隔离期守卫、可被分配器复用为它用；该复用的安全性由《形式化论证》第 4 章 L-NOREUSE 四情形论证承担（前提一「经检查写不可达锁槽」由 L-UNREACH（§4.9）给出、前提二「定向写排除」列入定理 5.1 前件，巧合 v=k 概率 ≤ 2⁻⁶⁴（见《形式化论证》§1.2、§5.5）），不属本局限。
