...
**Delete**（`ir.py:100`）。现状：bitcast 为 `i8*` 调 `free`，ZST 指针释放为 no-op。变化：前提 `is_heap(p) ∧ live(p) ∧ p.index = 0 ∧ is_origin(p)`（规则 3.6.2；前提求值顺序 `live`/`is_heap` 先于或并列 `index = 0`，且对 `lock_ptr = 0` 短路为假——`delete(null)` 确定性 trap；`is_origin` 定义见《形式化论证》§2.5），动作先 `μ(p.lock_ptr) := SENTINEL` 再块入隔离（内存保持可读、用户数据区逻辑上撤销、锁槽区留在 `dom(μ)`，隔离假设《形式化论证》§2.2）。检查插入：四前提检查（锁槽 kind 判定 `is_heap`，定义 9；键比较 `live`；`index = 0`；`is_origin` 读块头分配点记录）。结论：**需改造**（四检查 + 锁槽写哨兵）。补充：`is_heap` 与 `live` 对同一锁槽只做一次寻址、键比较共享读取结果，`is_origin` 另读块头分配点记录；释放范围按 `p.size·|T|` 计算；Malloc 写入块头、Delete 读取校验——重锚定子对象指针（`&s.field`、`&arr[i]`）`delete` 视为错误操作（trap），即使 `index = 0`；不再依赖根追踪（§8.4 降级为诊断用途）。
...
**隔离假设（out，实现选择边界）**：本机制要求释放后内存保持映射可读（不 `munmap`，《形式化论证》§2.2）：释放协议写 SENTINEL 后块入隔离、内存不归还 OS，使隔离期内锁槽读得确定性哨兵。若实现选择把释放内存归还操作系统（`munmap`），读已释放锁槽由 trap 退化为段错误，属实现选择边界（论证范围声明）。隔离期结束、块被复用后锁槽物理位置留在 $\mathrm{dom}(\mu)$、脱离隔离期守卫、可被分配器复用为它用；该复用的安全性由《形式化论证》第 4 章 L-NOREUSE 四情形论证承担，不属本局限。
