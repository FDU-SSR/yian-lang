...
- **形态对照**：实测时间开销随访问密度上升（1.30× → 2.74×），与 §10.6 预期形态一致；§10.6 的形态描述保留为对照基准，实测数字以本节与 `build/bench/results.md` 为准。正确性对照与负例集（§10.5 步骤 3-4）由 `tests/fat/`（55 用例）与 `tests/fat_cve/`（80 用例）回归套件持续验证，本节只回答开销目标与对照目标。
