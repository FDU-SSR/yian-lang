...
覆盖闭合：设计时 §7.1（11）+ §7.2（12）+ §7.3（24）= 47 类节点与 `ir.py` 全部节点类一一对应（nullptr 字面量节点随关键字移除，2026-08）；tiered-pointers 实施后节点类变化为——t2 移除运行时入口 4 类（YianArgc/YianArgvPtr/YianCstrlen/YianExit）、t3 新增 CheckRefAccess（§7.1）、既有检查/比较节点（Check*/PtrCmp/GenKey/WriteLockSlot/MemCopy）仍属 §7.1/§7.3 既有分类，覆盖闭合结论不变，无遗漏。
...
**实施状态（2026-08）**：结构转换 `bitcast` 与 `from_raw_parts` 切片视图站点已移除（`lib/` 全库 `bitcast` 清零；`from_raw_parts` 仅余 `lib/core/slice.an:65` 定义（`lib/core/env.an` FFI 站点已随 t2 移除））；字节级哈希已收窄为 `__memcpy` 受限名（实现取「保留为受限名」路径，未单列编译期内建字节视图 intrinsic）；`tests/std/` 测试套件豁免与受限名集合见 §8.3 实现注意。移除后 §7.1 Cast 留白（L22）的论证缺口收窄到「字节级哈希」一类，由 `__memcpy` 受限名承担，见 §11.2。指针比较字段化（§7.6 风险 3）已随 CFG 层插检实施（t10，Binary 指针分支 `PtrCmp`/`CheckPtrCmp`）：相等比较按 (data, index) 二元组、序比较先查 data 相等（跨对象 trap）；函数指针不参与。tiered-pointers 表示层（t2）与检查分级（t3）已落地（§7.4 三结构、§7.1 CheckRefAccess）；全量回归 253/253（主）+ 65/65（fat）+ 76/76（fat_cve）通过。
...
- **形态对照**：实测时间开销随访问密度上升（1.30× → 2.74×），与 §10.6 预期形态一致；§10.6 的形态描述保留为对照基准，实测数字以本节与 `build/bench/results.md` 为准。正确性对照与负例集（§10.5 步骤 3-4）由 `tests/fat/`（65 用例）与 `tests/fat_cve/`（76 用例）回归套件持续验证，本节只回答开销目标与对照目标。
