...
| P2 | ~~`optimize_checks`~~ **（P9 实测后取消）** | — | 原目标：只做"证明冗余后删除/合并"。P9 实测：CFG 层融合、发射层分支融合、表示层候选 B **三条路都无收益**（§5.7/§5.8），故该 pass 不建；检查插入 pass 直接接 `cleanup` |
...
- **候选 B（设计后实测否决，见 §5.8）**：让锚定指针的 live 直接读块头 key。设计要求把锚点
  放进指针自己（word 已满，需借判别位），而"锚点还原"两条指令与今天的下标算术等值，拿不到
  B1 的便宜 live；唯一能拿到的是回到 B1 的 16 B 指针布局——那是已经裁定过的取舍。
...
1. **决策迁移的等价性是最大风险**（P8 已消化）：18 条规则里任何一条漏掉都会变成"少检查"
   ...
   `cfg.txt` 逐字节等价收口；P9 最终没有产出任何序列改动（§5.7/§5.8），这条风险随之关闭。
2. **故意改变检查序列会改 LLVM 布局**（本仓库实测过 ±10% 的布局效应）：P9 的三个候选都实测为
   零收益或不可行，因此没有触发这条风险；纯重构阶段（P1–P8）按 §5.4 零漂移（P8 第 4 组实测
   micro A/B 漂移 ≤1.2%，P10 实测优化后 LLVM IR 逐字节相同）。
