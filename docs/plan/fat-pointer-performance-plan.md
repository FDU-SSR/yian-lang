...
- **合并相邻检查：已做**。相邻派生链上的良构 + `in_bounds` + `live` 合并为单个 `CheckElementAccess`，
  `(base, offset)` 二元检查去重，派生链失效时补发挂起义务（`cfg/passes/insert_checks.py`）。
- **恒真时序项的消解：已做**。
...
要么靠逃逸/过程间分析证明目标块不会被释放；表示侧的候选已评估并关闭（§4B），这条退路不存在。

### B. 指针表示与 ABI —— 不推进

这一方向的细化计划（`docs/plan/fat-pointer-representation-plan.md`）已移除，布局类候选不再列为计划：
32 位 `index`/`size` 与统一锁表已经落地，其余候选（16 B 打包字、8 B 字两变体、按标量传参的调用约定、
更小的指针 + 地址反查、末端地址）要么实测否决、要么收益缩水到不值得动。留档要点：

- **当前布局**：`T*` 24 B（`data, word, index:u32, size:u32`）、`T[]`/`str` 24 B、`T&` 16 B；
  堆块头 8 B（`{extent, pad}`）+ 锁表项 `{key:u32, anchor_lo32}`；raw 仍 8 B 裸指针。
- **尺寸就是 cache 足迹**：指针追逐的节点 16/24/32/40 B 分别是 5.51/7.42/9.25/10.99 ns/节点（C 微基准）。
- **传参没有可动空间**：LLVM 的 x86-64 后端本就把一等聚合参数按八字节拆进整型寄存器序列；
  C 侧"超 16 B 走栈"是 `byval` 语义，不能外推到 YIAN。
- **若要重开**：先量"撤销 32 位长度上限"与"视图构造/转换消解"各自能拿回多少，再决定是否动布局；
  `bench/fatptr/` 的三件微基准（`copy_struct`/`call_abi`/`chase`）与 `bench/results/` 的基线可直接用。
- 表示改动的同步面：`lockmech.py` 字段下标、`docs/security.md`、`docs/grammar/02.type_system.md`、
  `docs/manual/12.llvm_codegen.md`、`tests/` 里的 `@sizeof`/niche 断言。
...
**结论**：布局类候选已关闭（§4B）——32 位打包与统一锁表已落地，16 B 打包字 / 8 B 字两变体 /
按标量传参均实测否决，更小的指针（地址反查）与末端地址在今天的布局下已无尺寸收益。
**若重开**：先做两件度量——当前 `live` 的命中率与延迟占比、视图构造与转换在热点基准里的占比——
再决定是否动布局；验收仍按 §3.2 的协议，且 `bench/results/full.csv` 的 fat/raw 与 RSS 不退化。
...
- **剩余 `live` 开销在堆指针上**：指针追逐负载每访问一个对象就检查一次它的锁槽，局部无法证明该块
  未被释放。要么做逃逸/过程间分析；表示层的候选已关闭（§4B），不构成退路。
