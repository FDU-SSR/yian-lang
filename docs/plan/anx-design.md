...
- 与其它诊断不同，环是**致命错误**而不是收集起来的 `Diagnostic`：有环时没有可用的部分图，
  因此 `load()` 抛 `CycleError`。CLI 用目录里的码把它渲染成 `error[AX006]: ...`（不变量：
  环不是「多诊断」之一，§7.4）；`anx graph` 把同一个环写进它总是要打印的 JSON 载荷，
  而不是抛出 traceback。
...
| 层 | 产生的诊断 | 说明 |
| --- | --- | --- |
| `anx.load()` | `AX001`–`AX008`、`AX011`、`AX013`、`AX015` | 只覆盖项目与清单：发现、清单、依赖路径、环（`AX006` 以 `CycleError` 抛出，见 §4.2）、同名包、目录结构、保留名、源码根嵌套、bin 作依赖 |
