...
**P8 结论：采用候选 ②（依赖本机 Python）。** 本机已有 editable 安装，扩展只需要知道"用哪个解释器"；
随 VSIX 打包运行时（①）会把扩展变成平台相关的大包，独立可执行文件（③）要为 llvmlite 及其原生库做
逐平台打包，收益只落在"没有 Python 的机器"，而本仓库的使用者不是这类用户。与 §5.10 绑定的小决策取
**显式解释器**一侧：扩展默认仍启动 `PATH` 上的 `yian-lsp`，readme 推荐多环境机器把
`yian.languageServer.command` 设为解释器绝对路径、`args` 设为 `["-m", "lsp"]`（`python -m lsp` 与
`yian-lsp` 是同一个入口点）。版本匹配：`lsp/server.py` 的 `SERVER_VERSION` 与扩展 `package.json` 的
`version` 同号发布，服务器在 `initialize` 里回 `serverInfo`，扩展比对不一致时在输出通道警告并提示重装。
...
**状态**：已完成。级别 a（去抖 + 取消）与级别 b（按功能跳过 pass）都已落地，分发按 §6.1 的结论取
候选 ②，版本、变更日志、安装/卸载/回滚与故障排查写进 `ide-support/vscode/readme.md`，分析接口与
`lsp/` 写进 `docs/manual/26.analysis_interface.md`、`docs/manual/27.language_server.md`。
...
**实测与结论**（已完成；单机、热态、标准库 34 个文件加生成的包）：

| 路径 | 39 文件 | 95 文件 | 285 文件 |
| --- | --- | --- | --- |
| 打字反馈（只跑前端，级别 b） | 78 ms | 103 ms | 145 ms |
| 首次打开（完整前缀） | 132 ms | 263 ms | 582 ms |
| 悬停（快照就绪） | 10 ms | 13 ms | 20 ms |
| 补全（快照就绪） | 2 ms | 7 ms | 17 ms |

仅标准库（34 文件）的首次完整分析是 106 ms，常驻内存 32–56 MB。

- 打字反馈全部低于 §5.12 的"不可接受"线（200 ms），100 文件以内低于 100 ms；悬停与补全远低于
  100 ms。**达标，因此不进入级别 e（细粒度增量）**。
- 首次打开超过 200 ms 的规模下，代价的大头是标准库：34 个文件就占前端耗时约 60–76 ms。若将来要在
  几百文件规模把打字反馈压到 100 ms 以内，**下一个该做的是级别 c（stdlib 结果跨快照复用），而不是
  d/e**（§5.12 的判断顺序）。
- 级别 b 的实现方式：打字路径只做 lex → parse → desugar（`AnalysisSession.analyze(syntax_only=True)`），
  打开文件、保存与任何语义请求才跑完整前缀；语义 token 是被动特性，只读缓存，文本领先时返回空数组，
  客户端保留 TextMate 着色，完整分析落地后再发 `workspace/semanticTokens/refresh`。
