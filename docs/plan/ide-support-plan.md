...
**测量口径**：P2 起就用现成的 `--profile` / `timings` 机制给分析前缀加分段，P8 收口。要出四个数：
首次打开项目、修改后重分析、悬停、补全，判定用 P0 的 100ms / 200ms。**判断顺序写死：如果"首次打开"
就超过 200ms，问题几乎一定在 stdlib，该做的是 c 而不是 e**——避免一上手就往增量引擎走。
...
- **对 P0 冻结的指标下结论**：达标则关闭"是否需要增量"这个问题（§5.12），不达标则按 §5.12 的
  成本阶梯从 a 往 e 升，不跳级；
