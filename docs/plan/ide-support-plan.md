...
> 不重写语言前端**（§5.8）；**接入方式采用语言服务器（LSP）**（§5.9）。
...
2. **Parser 遇到错误立即停止**：解析器通篇 `raise ParseError(...)`，没有错误恢复；
   ...
   按"先做到不崩、能返回部分结果"分阶段做，不能一步改成多诊断（§6.2）。
...
打开文件后对当前内存文本做一次完整分析，先保证正确性。只有在 §6.3 的指标无法满足时才考虑
...
- **进程形态与分发成为下一个必须先定的事**（§6.1）：LSP 是独立进程，而 §6.1 的结论会决定分析
  会话是"进程内库调用"还是"子进程 + 协议"。这一步要在 P2 定接口之前给倾向。
- 语义高亮走 LSP 的 semantic tokens（§5.4 已按此设计）；位置转换层面向 LSP 的 0 基行与 UTF-16
  列（§5.1 已按此设计）。
- **分发成为显式风险**：服务器需要 Python 运行时与 `pygls`。P2/P3 先采用"依赖本机 Python
  （复用已有的 editable 安装）"，把打包形式留给 P8 决定（§6.1、§7 P8）。
...
位置已经定在顶层 `lsp/`（§5.7），接入方式已定为 LSP（§5.9），这里要定的是**进程形态与分发**：

- 进程形态候选：① `lsp/` 作为独立进程，扩展通过 stdio 启动它（LSP 的常规形态）；② 先用外部命令
...
- 解析器容错改造：先保证"遇到坏输入不抛异常、能返回部分结果 + 错误列表"，具体恢复策略
  （局部恢复、占位节点、保留上一次有效结果或组合）在选型后记入 §6.2（第 2 条）。
...
- **创建顶层 `lsp/` 包**（§5.7、§5.9）并同步 `pyproject.toml` 的三处配置；`lsp/` 只做协议适配
  与进程管理（stdin/stdout JSON-RPC），索引与项目模型仍从 `compiler/` / `anx/` 复用。进程形态与
  分发按 §6.1 的结论执行；
- 扩展侧接入 `vscode-languageclient`，把"启动/停止语言服务器 + 转发文档事件"跑通。
...
- 按 §6.2 的结论决定是否在本阶段引入多诊断（§5.8.2 第 3 条）。
...
- **对 P0 冻结的指标下结论**：达标则关闭"是否需要增量"这个问题（§6.3），不达标则先做快照粒度
  的优化，再评估细粒度增量；
