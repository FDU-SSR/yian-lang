...
- 同会话映射: ③−② 倍率 **0.99–1.22×**（14 基准中 11 个 ≤ 1.10×）；绝对成本仅 list（51500ms，占比 21.2%）、towers（8364ms，28.4%）、fasta（913ms，53.3%）显著，其余 ≤ 970ms。与 O2 时代旧树（spectralnorm 2.14×、list 2.22×）相比，lazy-lvalue-fat / bench-optimize-ptr 后检查成本大幅下降；Option<T&> 迁移后进一步下降（原 1.00–1.46×、list 58.3%）。注: fasta 913ms/53.3% 为 genelist 恢复 T* 修复后（2026-08-19 定向重测, check 5053.5 / nocheck 4140.7 / raw 3341.8）;修复消除聚合参数提取开销后检查成本成为 fasta 主导项（原 T[] 化 1.68× 超限数据已废弃）。
...
|---|---|---|---|---|
| ① 检查发射 | ③−② | icmp +52~89、trap +21~35、cfg 节点 18~27 | 比较 + 分支 → ud2，分支增量 77~131 | 0.99–1.22×；绝对仅 list 51500ms 显著 |
...
- **表示最重（queen 及全部基准的主导开销）: 聚合引用化 / 零拷贝**。栈上胖指针零拷贝传递（引用化）、字段访问合并（提取合并/往返消除）、回溯数组与链表的扁平化存储布局。表示成本占多数基准总成本 >80%，是当前第一优先。
...
- **动态频率型（list / towers / fasta，检查占比 21–53%）: 热循环检查合并 + 字段复用**。`is_shorter_than` 内单次取 data / size 复用，避免逐字段 extractvalue；链表节点用邻接/连续表示减少索引路径；检查在已被证明非空的循环中下沉。（fasta 修复后检查占比升至 53.3%，为三人组最高，见 §2①。）
...
| 倍率（③/①） | 3.22 / 3.64 / 3.42×（中位数） | 1.87 / 2.48 / 2.06× | 下降主因: 当前树 lazy-lvalue-fat / bench-optimize-ptr 使检查成本 collapse（③−② 由 ~2.1× 降至 1.0–1.5×，迁移后再降至 0.99–1.22×）；旧文档 §5 已注明"IR 新生成，绝对时间不对齐属预期" |
...
2. **开销层面**: **胖表示**（extractvalue / 40B 聚合）为当前主导开销（②−① = 0.99–1.99×，多数基准占总成本 >80%，list 191559ms / 78.8%、queen 23588ms / 93.3%）；**检查发射**（比较 + 分支 → trap）次之（③−② = 0.99–1.22×，仅 fasta 1.22× 超 1.2×——修复后检查占比 53.3%，list / towers / fasta 绝对显著）；锁槽帧锁与元数据传递为含在表示内的次级项。与 O2 时代旧树相比，检查成本经 lazy-lvalue-fat / bench-optimize-ptr 大幅下降，Option<T&> 迁移（节点 Option 40B→24B）后表示成本占比进一步上升。
