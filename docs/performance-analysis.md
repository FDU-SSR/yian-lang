...
重测后（数据见 `docs/shootout-results.md` 新表）: **14/14 基准 raw ≤ check**（check/raw = 1.00–2.84×），其中 13 个 > 1.00×（mand 1.00× 持平）。0.47× 级假象消失。同时 raw 绝对值回到 O2 时代水平（list raw 43276→21347ms、fann 6239→3076ms），确认假象会话 raw 段整体慢 ~2.1×。

## 1. 三层分析方法

分析针对三个代表性基准: spectralnorm（静态检查密度冠军）、queen（表示成本绝对冠军）、list（总成本绝对冠军）。选取依据为同会话成本分解（表 1）。

表 1: TOP 基准选取（三态紧邻重测成本分解, 倍率 vs ①raw）

| 基准 | 表示(②/①) | 检查(③/②) | 总(③/①) | 选取理由 |
|---|---|---|---|---|
| **spectralnorm** | 1.81× | 1.04× | 1.89× | 静态检查密度冠军（cfg 27 节点、icmp 检查差分 +81；检查占比现仅 8.7%，成本已由表示主导） |
| **queen** | 2.44× | 1.02× | 2.49× | 表示成本绝对冠军（23493ms，占总成本 96.3%） |
| **list** | 1.49× | 1.46× | 2.17× | 总成本绝对冠军（24978ms）+ 绝对检查成本冠军（14551ms）+ 检查占比冠军（58.3%） |
...
perf 层单次同会话采样与三态紧邻 5 次中位数高度吻合（1.90/2.49/2.20 vs 1.89/2.49/2.17），相互印证——perf 层本就同会话执行，未受假象影响。
...
- 同会话映射: ③−② 倍率 **1.00–1.46×**（14 基准中 11 个 ≤ 1.10×）；绝对成本仅 list（14551ms，占比 58.3%）、towers（4028ms，44.5%）、fasta（895ms，48.4%）显著，其余 ≤ 970ms。与 O2 时代旧树（spectralnorm 2.14×、list 2.22×）相比，lazy-lvalue-fat / bench-optimize-ptr 后检查成本大幅下降。
...
- 同会话映射: ②−① 倍率 **1.00–2.44×**（多数基准 > 1.3×）；绝对成本 queen **23493ms**（占比 96.3%）为绝对冠军，其次 sieve 14064ms、list 10427ms、permute 9053ms。**表示成本现为全部基准的主导开销**。
...
|---|---|---|---|---|
| ① 检查发射 | ③−② | icmp +52~89、trap +21~35、cfg 节点 18~27 | 比较 + 分支 → ud2，分支增量 77~131 | 1.00–1.46×；绝对仅 list 14551ms 显著 |
| ② 胖表示 | ②−① | extractvalue +99~164（最大单项）、gep +20~40 | mem 访问增量 190~323（list 4.7×、queen +323） | 1.00–2.44×；queen 23493ms 绝对冠军 |
...
三层证据: 静态检查密度三人组最高（cfg 27 节点、icmp 检查差分 +81、asm 分支 160）；热点 `index` / `mult_Av` / `mult_Atv` 全为矩阵乘内逐元素访问。但同会话测量显示检查占比仅 **8.7%**（③−② = 1.04×，652ms），总成本 1.89×（7476ms）中表示成本占 91.3%（1.81×）。
...
三层证据: 同会话表示成本 **2.44×**（23493ms，占总成本 96.3%，绝对冠军）；asm 增幅最大（2.7×，+1123 行）、mem 内存访问绝对增幅最大（+323）；热点 `is_safe` / `index` / `place_queen` 为回溯热路径，40B 胖指针频繁构造/析构（update_board_state / place_queen）。检查成本仅 1.02×（892ms）。

瓶颈: 回溯状态下标与棋盘数组的胖表示存取；`is_safe` 的 33.43% 自占 + `index` 27.90%。

### 3.3 list: 总成本 + 绝对检查成本最高（动态频率型）

三层证据: 同会话总成本 **24978ms**（绝对冠军）、绝对检查成本 **14551ms**（占比 58.3%，全部基准最高）；但**静态检查密度三人组最低**（cfg 18 节点、icmp 检查差分 +52、asm 分支 107）。瓶颈不在密度而在**执行频率**: `is_shorter_than` 87.67% 自占，链表 O(n²) 两两比较，每步都对胖指针取字段并做检查。

定位: 静态 IR 密度 × 海量动态执行次数 = 最大绝对成本；本轮为唯一"检查成本与表示成本相当"（1.46× vs 1.49×）的基准，与 spectralnorm / queen 的"表示主导"形成对照。

表 8: 三基准瓶颈对照

| 基准 | 瓶颈形态 | 关键证据 | 热点（Self%） |
|---|---|---|---|
| spectralnorm | 静态密度型（检查密度最高，成本已转表示主导） | 检查占比 8.7%、cfg 节点 27、表示 1.81× | `index` 24.18% |
| queen | 表示型（表示 + 元数据传递） | 表示 23493ms（96.3%）、asm 2.7×、mem +323 | `is_safe` 33.43% |
| list | 动态频率型（检查） | 总成本 24978ms、绝对检查 14551ms（58.3%）、静态密度最低 | `is_shorter_than` 87.67% |
...
- **动态频率型（list / towers / fasta，检查占比 ≥44%）: 热循环检查合并 + 字段复用**。`is_shorter_than` 内单次取 data / size 复用，避免逐字段 extractvalue；链表节点用邻接/连续表示减少索引路径；检查在已被证明非空的循环中下沉。
...
|---|---|---|---|
| 三态关系 | 14/14 raw ≤ check（check/raw 1.47–4.34×） | 14/14 raw ≤ check（1.00–2.84×） | 一致（假象会话的 9 个 ≤1.00× 消失） |
| 检查成本排序 | list 绝对冠军（旧树 36211ms） | list 仍绝对冠军（14551ms，占比 58.3%） | 一致；绝对量因检查优化大幅下降 |
| 表示成本 | queen 绝对冠军（旧树 28570ms） | queen 仍绝对冠军（23493ms，96.3%） | 一致 |
| 倍率（③/①） | 3.22 / 3.64 / 3.42×（中位数） | 1.89 / 2.49 / 2.17× | 下降主因: 当前树 lazy-lvalue-fat / bench-optimize-ptr 使检查成本 collapse（③−② 由 ~2.1× 降至 1.0–1.5×）；旧文档 §5 已注明"IR 新生成，绝对时间不对齐属预期" |
| perf 单次同会话 | 1.90 / 2.49 / 2.20×（2026-08-17） | 紧邻中位数 1.89 / 2.49 / 2.17× | 高度吻合（perf 层本就同会话，未受假象影响） |

无数值冲突；O3 报告（4230dc0）的 0.47× 级"胖快于裸"已被 §0 判定为跨会话测量假象，本版数据不再包含该偏差。
...
1. **测量层面**: O3 报告中的"胖快于裸"（9 个基准 ≤ 1.00×）为跨会话测量假象——raw 独立会话期间系统慢 ~2.1×（同一二进制 O2/O3 逐字节相同，时间却慢 2.11–2.28×，fann/nbody/mand 恰 2.177×）。三态紧邻重测（方案 A）后还原真实关系: 14/14 基准 raw ≤ check（1.00–2.84×）。
2. **开销层面**: **胖表示**（extractvalue / 40B 聚合）为当前主导开销（②−① = 1.00–2.44×，多数基准占总成本 >80%，queen 23493ms / 96.3%）；**检查发射**（比较 + 分支 → trap）次之（③−② = 1.00–1.46×，仅 list / towers / fasta ≥ 1.2× 显著）；锁槽帧锁与元数据传递为含在表示内的次级项。与 O2 时代旧树相比，检查成本经 lazy-lvalue-fat / bench-optimize-ptr 后大幅下降。
3. 三类基准呈现两种瓶颈形态: **表示主导型**（queen / spectralnorm: 表示成本占比 >90%，热点在胖指针辅助函数 index / ptr / as_struct）与**动态频率型**（list: 检查占比 58.3% 冠军，is_shorter_than 87.67% 海量执行）。
